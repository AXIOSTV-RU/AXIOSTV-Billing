...
=head1 NAME
  ...
  AXbills Console document creator
...
sub prepaid_invoices_company {
  ...
  foreach my $line (@{$list}) {
    my $name = $line->{company_name};
    my $deposit = $line->{deposit};
    my $admin_user = $line->{uid};
    my $admin_login = $line->{login};
    my $admin_user_email = $line->{email} || '';

    print "COMPANY: $name UID: $admin_user DEPOSIT: $deposit\n" if ($debug > 2);
