#!/bin/sh
# AXbills IPN filter example
# ARGUMENTS:
# %STATUS (ONLINE_ENABLE,ONLINE_DISABLE,HANGUP) %LOGIN %IP %FILTER_ID [-debug]
# $conf{IPN_FILTER}='/home/asmodeus/axbills2/misc/ipn_filter.sh %STATUS "%LOGIN" "%IP" "%FILTER_ID" "%UID" debug';
...
if [ w$4 = w ]; then
  ...
  echo "AXbills IPN filter $version. Debug mode: ${debug}";
