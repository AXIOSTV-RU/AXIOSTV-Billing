...
=head1 NAME
  ...
  Миграция в AXbills
   ...
   2axbills.pl former AXbills migration file for Cards module account creation
...
if ($from) {
  ...
  elsif ($from eq 'axbills') {
    $INFO_LOGINS = get_axbills();
  }
...
sub db_connect {
  ...
  my $dbname = $attr->{DB_NAME} || "axbills";
  ...
  if ($attr->{ABILLS_DB}) {
    if (!-f '/usr/axbills/libexec/config.pl') {
      print "Can't find /usr/axbills/libexec/config.pl

AXbills not installed\n";
    ...
    if ($@) {
      ...
      print "Manual: https://billing.axiostv.ru/display/39/Perl%3A%3AODBC\n";
      exit;
    }
    DBD::ODBC->import();
...
sub add_nas {
  ...
  unshift(@INC, $Bin . '/../', $Bin . '/../AXbills', $Bin . "/../AXbills/$conf{dbtype}");
  ...
  my $axbills_db = AXbills::SQL->connect($conf{dbtype}, $conf{dbhost}, $conf{dbname}, $conf{dbuser},
    $conf{dbpasswd}, { CHARSET => ($conf{dbcharset}) ? $conf{dbcharset} : undef });

  my $admin = Admins->new($axbills_db, \%conf);
  $admin->info($conf{SYSTEM_ADMIN_ID}, { IP => '127.0.0.1' });
  #my $Dhcphosts = Dhcphosts->new($axbills_db, $admin, \%conf);
  $admin->{MODULE} = '';
  my $Nas = Nas->new($axbills_db, \%conf);
...
sub help {
  ...
  print <<"[END]";
AXbills Система миграции: $VERSION
(https://billing.axiostv.ru)
  ...
  Options:
    ...
    FROM                - Migration From:
                            ...
                            axbills    - get users from another axbills
                            ...
                            stargazer_pg - stargazer (Ubilling) Postgree DB
                            carbon4
                            carbon5
                            lms
                            ...
                            nika
                            mikrobill
    SYNC_DEPOSIT        -  filename to sync deposit ( ./2axbills.pl FROM=file SYNC_DEPOSIT=/usr/deposits )
...
sub mikbill_pools {
  ...
  my $sql = "INSERT INTO axbills.ippools (name, netmask, ip, dns, gateway, vlan, comments, static, counts)
...
sub mikbill_payments {
  ...
  my DBI $db_axbills = db_connect({ ABILLS_DB => 1 });
  my $sql = qq{
  SELECT
users.user AS login,
...
sub get_carbon4 {
  ...
  foreach my $user (values %{$users_list}) {
    ...
    # Translating carbon 'ENABLED' to axbills 'disabled' attr
...
sub get_nika {
  ...
  my $sql = qq{
TRUNCATE TABLE axbills.groups;
TRUNCATE TABLE axbills.users_contacts;
SET FOREIGN_KEY_CHECKS = 0;
TRUNCATE TABLE axbills.users;
TRUNCATE TABLE axbills.bills;
TRUNCATE TABLE axbills.docs_invoice2payments;
TRUNCATE TABLE axbills.payments;
TRUNCATE TABLE axbills.fees;
TRUNCATE TABLE axbills.docs_invoice_orders;
TRUNCATE TABLE axbills.docs_invoices;
TRUNCATE TABLE axbills.docs_receipt_orders;
TRUNCATE TABLE axbills.docs_receipts;
TRUNCATE TABLE axbills.maps_points;
TRUNCATE TABLE axbills.builds;
TRUNCATE TABLE axbills.admin_actions;
TRUNCATE TABLE axbills.admin_system_actions;
SET FOREIGN_KEY_CHECKS = 1;
TRUNCATE TABLE axbills.internet_main;
TRUNCATE TABLE axbills.districts;
TRUNCATE TABLE axbills.streets;
TRUNCATE TABLE axbills.users_pi;
TRUNCATE TABLE axbills.abon_tariffs;
TRUNCATE TABLE axbills.abon_user_list;

SET SQL_MODE=NO_ENGINE_SUBSTITUTION;

#GROUPS
INSERT INTO axbills.groups (gid, name, descr)   SELECT     id,     name,     comment   FROM     nika_system.groups;
#Users
INSERT INTO axbills.users (uid, id,gid, disable, bill_id, registration, password)
...
#Bills
INSERT INTO axbills.bills(id,uid,deposit,registration) SELECT id, id, depozit, datetime FROM nika_system.abon;

INSERT INTO axbills.payments( date, sum, ip, uid, aid, inner_describe ) SELECT NOW(), depozit, INET_ATON('127.0.0.1'),id,2,'Migration' FROM nika_system.abon;

#Internet
INSERT INTO axbills.internet_main (uid, ip, cid, port) SELECT user, INET_ATON(ip), mac, port FROM nika_system.comp;

UPDATE axbills.internet_main i

  CROSS JOIN
  ( SELECT if(tp.tp_id IS NULL, 111, tp.tp_id) as insert_tp_id, a.id AS nika_uid
    FROM axbills.tarif_plans tp
      LEFT JOIN nika_system.abon a ON (a.tarif=tp.id)
  ) AS m
SET i.tp_id=m.insert_tp_id
WHERE i.uid=m.nika_uid;

INSERT INTO axbills.internet_main (uid, ip, cid, tp_id, comments) SELECT 2, INET_ATON(ip), mac, 1, CONCAT(name_m, '', type) FROM nika_system.modem WHERE ip LIKE '172.17.%';

INSERT INTO axbills.abon_tariffs (id, name, period, price)   SELECT id, name,  IF(dayORmonth='day', 0, 1), IF(dayORmonth='day', cost_day, cost) FROM nika_system.new_tarif WHERE id IN (183, 182, 165, 164, 163, 157, 156, 155, 154);

#Address
INSERT INTO axbills.users_pi (uid, fio, fio2, fio3, email, comments, pasport_grant, pasport_num, address_flat, contract_id) SELECT id, first_name, second_name, third_name, email, comment, pass_data, pass, apartment, id FROM nika_system.abon;
UPDATE axbills.users_pi SET address_street = (SELECT nika_system.streets.street
                                              FROM nika_system.streets
                                              LEFT JOIN nika_system.abon ON (nika_system.abon.street=nika_system.streets.id)
                                              WHERE (nika_system.abon.id=axbills.users_pi.uid));

UPDATE axbills.users_pi SET address_flat = (SELECT apartment FROM nika_system.abon WHERE (nika_system.abon.id=axbills.users_pi.uid));
UPDATE axbills.users_pi SET address_build = (SELECT house FROM nika_system.abon WHERE (nika_system.abon.id=axbills.users_pi.uid));

INSERT  INTO axbills.districts (`name`, `comments`) VALUES ('Main District', '');
REPLACE INTO axbills.streets (name, district_id) SELECT address_street, 1 FROM axbills.users_pi group by 1;
REPLACE INTO axbills.builds (street_id, number) select s.id, address_build from axbills.users_pi u, axbills.streets s   WHERE u.address_street=s.name GROUP BY address_street, address_build;

UPDATE axbills.users_pi SET _actual_mob = (SELECT actual
                   FROM nika_system.abon
                   WHERE (nika_system.abon.id = axbills.users_pi.uid));

UPDATE axbills.users_pi SET _actual_mob = (SELECT actual FROM nika_system.abon WHERE (nika_system.abon.id = axbills.users_pi.uid));

INSERT INTO axbills.users_contacts (uid, type_id, value) SELECT id, 2, phone FROM nika_system.abon  WHERE phone<>'';
INSERT INTO axbills.users_contacts (uid, type_id, value) SELECT id, 1, mobile FROM nika_system.abon WHERE mobile<>'';

UPDATE axbills.users_pi pi
  LEFT JOIN axbills.streets s ON (s.name=pi.address_street)
  LEFT JOIN axbills.builds b ON (s.id=b.street_id AND b.number=pi.address_build)
SET pi.location_id=b.id   WHERE pi.location_id=0;


UPDATE _connection_type_list SET id=12 WHERE id=11;
UPDATE axbills.users_pi SET _connection_type = (SELECT category FROM nika_system.abon WHERE (nika_system.abon.id = axbills.users_pi.uid));


INSERT INTO axbills.fees (uid, bill_id, dsc, date, sum, last_deposit)  SELECT s.user, s.user, s.tarif, s.date_start, s.summa, s.summa FROM stat_serv_new s;
