...
=head2 result_former($attr) - Make result table from different source
  ...
  Examples:
	https://billing.axiostv.ru/pages/viewpage.action?pageId=4358162
...
sub _get_search_titles {
  ...
  if ($permissions{0} && $permissions{0}{26}) {
    ...
    $SEARCH_TITLES{address_flat_alt} = $lang{ADDRESS_FLAT_ALT};
...
sub _column_no_permitss {
  ...
  if ($permissions{0} && !$permissions{0}{26}) {
    ...
    delete $LIST_PARAMS{ADDRESS_FLAT_ALT} unless ($hash_fields_options{ADDRESS_FLAT_ALT});
