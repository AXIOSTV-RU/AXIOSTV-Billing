...
sub leases_add {
  ...
  foreach my $lease (@{$leases_list}) {
    ...
    my $cmd = "/ip dhcp-server lease add address=$lease->{ip} mac-address=$lease->{mac} server=$dhcp_server_name disabled=no $address_list_arg comment=\"AXbills generated\"";
    push(@cmd_arr, $cmd);
  }
...
sub add_ssh_bruteforce_protection {
  ...
  $self->add_firewall_rule({
    ...
    action             => 'drop',
    comment            => "AXbills. Drop ssh bruteforces"
  });
