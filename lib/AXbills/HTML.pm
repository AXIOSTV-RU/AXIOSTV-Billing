...
sub new {
  ...
  $self->{OUTPUT} = '';
  %FORM = form_parse();
  unless ($CONF->{ALLOW_WEB_DEBUG} && ($ENV{REMOTE_ADDR} || '') =~ /^(127\.0\.0\.1|::1)$/) {
    delete $FORM{DEBUG};
  }
  _reject_foreign_post();
  get_cookies();
  $self->{HTML_FORM} = \%FORM;
  $SORT = 1;
...
#**********************************************************
=head2 _reject_foreign_post()

  Drop cross-site state-changing requests. Payment and API scripts
  are skipped.

=cut
#**********************************************************
sub _reject_foreign_post {
  return 1 if (($ENV{REQUEST_METHOD} || '') !~ /^(POST|PUT|PATCH|DELETE)$/);

  my $script = $ENV{SCRIPT_NAME} || '';
  return 1 if ($script =~ /paysys|api\.cgi|telegram|viber|wrt_configure/i);

  my $host = $ENV{HTTP_HOST} || '';
  $host =~ s/:\d+$//;
  return 1 if (!$host);

  foreach my $src ($ENV{HTTP_ORIGIN}, $ENV{HTTP_REFERER}) {
    next if (!$src);
    if ($src !~ m{^https?://\Q$host\E(?::\d+)?(?:/|$)}i) {
    ...
    foreach my $pair (@pairs) {
      ...
      if (defined($value)) {
        ...
        if (!$self->{SKIP_QUOTE}) {
          ...
          $value =~ s/\'/\\\'/g;
          $value =~ s/&rsquo;/\\\'/g;
          $value =~ s/<\s*script\b[^>]*>.*?<\s*\/\s*script\s*>//gis;
          $value =~ s/<\s*script\b[^>]*>//gi;
          $value =~ s/javascript\s*://gi;
        }
      }
      else {
        $value = '';
...
sub set_cookies {
  ...
  my $secure_flags = $self->{secure} || $attr->{SECURE} || '';
  $secure_flags .= '; HttpOnly' unless ($secure_flags =~ /HttpOnly/i);

  my $https = ($ENV{HTTPS} && $ENV{HTTPS} =~ /on/i)
    || ($ENV{HTTP_X_FORWARDED_PROTO} && $ENV{HTTP_X_FORWARDED_PROTO} =~ /https/i);
  $secure_flags .= '; Secure' if ($https && $secure_flags !~ /Secure/i);
  $secure_flags .= '; SameSite=Lax' unless ($secure_flags =~ /SameSite/i);

  $cookie .= " $secure_flags" if ($secure_flags);
