...
BEGIN {
  ...
  if (!%conf) {
    ...
    print "Create ACP config file /usr/axbills/libexec/config.pl\n";
    exit;
  }
...
sub quick_functions {
  ...
  if ($uid > 0) {
    ...
    if ($FORM{REFERER} && $FORM{REFERER} =~ /^\Q$SELF_URL\E/ && $FORM{REFERER} !~ /index=1000/) {
...
sub form_info {
  ...
  if ($user->{CELL_PHONE_ALL}) {push @cellphones, $user->{CELL_PHONE_ALL};}

  $contacts{PHONE} = ($#phones > -1) ? join(', ', @phones) : q{};
  $contacts{CELL_PHONE} = ($#cellphones > -1) ? join(', ', @cellphones) : q{};
  $contacts{EMAIL} = $user->{EMAIL_ALL} || q{};
    
	if ($contacts{CELL_PHONE}) {
    $user->{EXT_DATA_CP} = $html->tpl_show(templates('form_client_ext_cell_phone'), $user, { OUTPUT2RETURN => 1 });
  }
  
  	if ($contacts{PHONE}) {
    $user->{EXT_DATA_P} = $html->tpl_show(templates('form_client_ext_phone'), $user, { OUTPUT2RETURN => 1 });
  }
  
    if ($contacts{EMAIL}) {
    $user->{EXT_DATA_E} = $html->tpl_show(templates('form_client_ext_email'), $user, { OUTPUT2RETURN => 1 });
  }
	### END AXbills
  ...
  if (in_array('Internet', \@MODULES)) {
    ...
    $LIST_PARAMS{UID} = $user->{UID};
    $LIST_PARAMS{SHOW_ONLY_HOLDUP} = 1;
    internet_user_info();
  }
	### END AXbills
  ...
  if($conf{ADDRESS_REGISTER}){
    if (defined($user->{FLOOR}) || defined($user->{ENTRANCE})) {
      $user->{EXT_ADDRESS} = $html->tpl_show(templates('form_ext_address'),
        { ENTRANCE => $user->{ENTRANCE} || '', ADDRESS_FLAT_ALT => $user->{ADDRESS_FLAT_ALT} || '', FLOOR => $user->{FLOOR} || '' },
        { OUTPUT2RETURN => 1 });
    }
...
sub user_pi_change {
  ...
  my @fields_allow_to_change;
  if ($conf{CHECK_CHANGE_PI}) {
    @fields_allow_to_change = split(/,\s?/, $conf{CHECK_CHANGE_PI});
  }
  else {
    @fields_allow_to_change = qw(
      FIO FIO1 FIO2 FIO3 PHONE CELL_PHONE EMAIL
      ADDRESS_FLAT ADDRESS_BUILD ADDRESS_STREET STREET_ID LOCATION_ID
      DISTRICT_ID ADD_ADDRESS_BUILD ADDRESS_FLOOR ADDRESS_ENTRANCE
    );
  }

  my %safe_pi = (UID => $user->{UID});
  foreach my $key (@fields_allow_to_change) {
    next if (!$key);
    $safe_pi{$key} = $attr->{$key} if (defined $attr->{$key});
  }

  $user->pi_change(\%safe_pi);
  if ($user->{errno}) {
    if ($user->{errno} == 21) {
...
sub form_fees {
  ...
  if (!$FORM{sort}) {
    ...
    $LIST_PARAMS{PAGE_ROWS} = $attr->{rows} || $conf{list_max_recs};
  }
  ...
  $conf{user_payment_journal_show}//=12;
...
sub service_info {
  ...
  my $uid = $user->{UID} || $LIST_PARAMS{UID} || 0;
...
sub fl {
  ...
  if ($conf{user_finance_menu}) {
    push @m, "40:0:<p>$lang{FINANCES}</p>:form_finance:::";
...
sub _login_confirm_pin {
  ...
  return if ($FORM{UID} !~ /^\d+$/ || $FORM{PIN_CODE} !~ /^\d{4,12}$/);
  ...
  if ($user->{TOTAL} != 1) {
    ...
    my $phone_owns_pin = 0;
    foreach my $contact (@{$contacts}) {
      $phone_owns_pin = 1 if ($contact->{uid} && $contact->{uid} == $FORM{UID});
    }
    if (!$phone_owns_pin) {
      $params->{message} = $lang{CODE_IS_INVALID};
    }
      ...
      foreach my $contact (@{$contacts}) {
        next if (!$contact->{uid} || $contact->{uid} != $FORM{UID});
...
sub pin_code_generate {
  return mk_unique_value(8, { SYMBOLS => '0123456789' });
}
