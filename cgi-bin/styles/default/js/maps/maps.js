...
if (LAYERS && typeof(LAYERS) !== 'object') LAYERS = LAYERS.split(',');
...
var Configuration = (function () {
  ...
  function hasAddFunction(layer) {
    ...
    return (typeof (layer['add_func']) !== 'undefined' || LayersWithAddFunc.includes(layer.id.toString()));
  }
...
var ObjectsConfiguration = (function () {
  ...
  function showObject(layer, objects, err_callback, success_callback) {
    ...
    else if (objects[0] && objects[0]['GPS']) {
      let layer_id = 'layer_' + layer['id'].toString();
      if (!AllLayers[layer_id]) AllLayers[layer_id] = new L.FeatureGroup();

      jQuery.each(objects, function (index, object) {
        AllLayers[layer_id].addLayer(new GPS(layer.id, object.GPS));
      });
    }
...
class GPS {
  constructor(layer_id, params) {
    ...
    this._exportFunction = Layers[this._layerId].export_function;
    this._module = Layers[this._layerId].module;
    this._carIcon = L.divIcon({
      className: "leaflet-data-marker",
      html: L.Util.template(params.SVG),
      iconSize: [30, 30],
      iconAnchor: [13, 19],
      popupAnchor: [-10, -2]
    });
    this._group = new L.FeatureGroup();
    this._group._aid = this._aid;
  ...
  createRoute(){
    let self = this;
    Object.entries(self._coords).forEach(([date, value]) => {
      let myMovingMarker = L.Marker.movingMarker(value.coords, 9000, {
        autostart: 1,
        loop: 1,
        icon: self._carIcon,
        info: { points: value.info },
        params: self._params
      }).addTo(map);
      ...
      for (let i = 1; i < value.coords.length; i++) {
        self._group.addLayer(L.polyline([value.coords[i - 1], value.coords[i]], { color: self._color }).addTo(map));
        self._group.addLayer(L.polyline([value.coords[i - 1], value.coords[i]], { color: value.speed[i] < 61 ? 'green' : 'red', dashArray: '10,20' }).addTo(map));
      }

      self._group.addLayer(myMovingMarker);
      self._group.addLayer(myMovingMarker._points);

      if (self._params.flyToRoute) map.flyToBounds(value.coords);

      myMovingMarker._points.on('popupopen', function() { self.routeButton() });
    });
  }

  routeButton() {
    let self = this;
    jQuery(`.route-${self._aid}`).on('click', function() {

      let dateModal = new AModal();
      dateModal
        .setBody(self._modalBody())
        ...
        .show(function () {
          ...
          jQuery('#confirmRouteDate').on('click', function () {
            ...
            self._routeByDate(fromDate, toDate);
            dateModal.hide();
          });
        });
    });
  ...
  _routeByDate(fromDate, toDate) {
    let self = this;
    let layer = AllLayers[`layer_${self._layerId}`];

    jQuery.post(selfUrl, {
      header: 2,
      get_index: 'maps_get_objects',
      EXPORT_LIST: 1,
      RETURN_JSON: 1,
      MODULE: self._module,
      FUNCTION: self._exportFunction,
      AID: self._aid,
      FROM_DATE: fromDate,
      TO_DATE: toDate
    }, function(result) {
      ...
      if (jsonResult.length < 1) {
        displayJSONTooltip({MESSAGE: {caption: _NO_MOVES_FOUND, message_type: 'warn'}});
        return;
      }

      Object.entries(layer._layers).forEach(([id, subLayer]) => {
        if (!subLayer._aid || subLayer._aid !== self._aid) return;
        layer.removeLayer(subLayer);
      });

      jsonResult.forEach(object => {
        object.GPS.flyToRoute = true;
        AllLayers[`layer_${self._layerId}`].addLayer(new GPS(self._layerId, object.GPS));
      });
    });
  }

  _modalBody() {
    return this._dateFormGroup(true) + this._dateFormGroup();
  }

  _dateFormGroup(from = false) {
    let id = from ? 'FROM' : 'TO';
    ...
    label.innerText = from ? _FROM : _TO;
    ...
    input.pattern = "^\\d\\d\\d\\d-(0?[1-9]|1[0-2])-(0?[1-9]|[12][0-9]|3[01]) (00|0?[0-9]|1[0-9]|2[0-3]):([0-9]|[0-5][0-9])$";
    input.id = `${id}_DATE`;
...
var Markers = (function () {
  function addNewMarker(layer, lat, lng, layer_index) {
    ...
    if (!FORM['ADD_POINT'] || !FORM['OBJECT_ID']) {
      let url = '';
      if (FORM['GET_LOCATION']) {
        url = 'index.cgi?get_index=' + layer['add_func'] + '&header=2&add=1&ADD_EXIST_OBJECT=1&COORDX=' +
          lat + '&COORDY=' + lng + '&POINT_ID=' + FORM['OBJECT_ID'] + '&COORDINATES=1';
      } else {
        url = 'index.cgi?get_index=' + layer['add_func'] + '&header=2&add_form=1&TEMPLATE_ONLY=1&IN_MODAL=1&OBJECT_TYPE_ID=1&ADD_OBJECT=1';
      }
      mainLocation.askCustomMarker(lat, lng, url, function () {
    ...
    if (layer['id'] === LAYER_ID_BY_NAME['BUILD']) {
      ...
      let add_func = layer['add_func'] || 'maps_add_external_points';
...
var ChangeElement = (function () {
  ...
  function saveElement(object) {
    ...
    map.on(L.Draw.Event.EDITED, function (e) {
      if (elementEditor === undefined) return;

      map.closePopup();
      let result = {};
      jQuery.each(e.layers._layers, function (index, value) {
        if (result['latLngStr']) return;
      ...
      Configuration.sendFetch(link, () => { TempVariables['EDITED'] = undefined }, function () {
  ...
  function cancelElement(object) {
    ...
    map.closePopup();
    let oldPolyline = Objects[object.layer_id][object.object_id];
    ObjectsConfiguration.getObjects(Layers[object.layer_id], () => {
        TempVariables['EDITED'] = undefined;
      },
      function () {
        TempVariables['EDITED'] = undefined;
  ...
  function createCloseTraceBtn() {
    let closeBtn = L.Control.extend({
      ...
      _createHrefContainer: () => {
        ...
        _containerIcon.classList.add('fa-times');
...
var init_map = function () {
  ...
  L.Control.Watermark = L.Control.extend({
    onAdd: function () {
      ...
      href.href = MAPS_WATERMARK_URL || 'https://billing.axiostv.ru/';
      href.target = '_blank';

      let img = L.DomUtil.create('img', 'axbills_href', href);

      img.src = '/images/maps/icons/' + (MAPS_WATERMARK_ICON || 'acp.png');
...
let loadLayers = function () {
  ...
  fetch(selfUrl + '?header=2&get_index=maps_layers_list&RETURN_JSON=1')
    .then(response => {
      if (!response.ok) {
        throw response
      }
