...
function defineMainSearchLiveLogic() {
  ...
  let [main_field, second_field] = (_UNIVERSAL_SEARCH_FIELDS || 'address_full,phone').split(',');

  var login_uid_row_template = ''
      + '<span>&nbsp;{{fio}}</span>'
      + '<span class="float-right"><strong>&nbsp;{{login}} ( {{uid}} )</strong></span>';
  var company_row_template = ''
      + '<span><strong>&nbsp;{{company_name}}</strong></span><br/>';
  var extra_row_template = ''
      + `<br/><span>&nbsp;{{${main_field}}}&nbsp;</span>`
      + `<span class="float-right"><i class="text-muted">{{${second_field}}}</i></span>`;

  Mustache.parse(login_uid_row_template);
  Mustache.parse(company_row_template);
  Mustache.parse(extra_row_template);
      ...
      $input.marcoPolo({
        ...
        formatItem: function (data, $item) {
          ...
          if (data.company_name) {
            result += Mustache.render(company_row_template, data);
          }
          result += Mustache.render(login_uid_row_template, data);
