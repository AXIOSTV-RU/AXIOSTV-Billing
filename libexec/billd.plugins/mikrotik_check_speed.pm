    ...
    if ( $argv->{RECONFIGURE} ) {
      ...
      if ( $argv->{NAT} ) {
        push @commands, qq{/ip firewall nat remove [find src-address-list~"^CLIENTS"]},
          ...
          qq{/ip firewall nat add chain=srcnat src-address-list="CUSTOM_SPEED" action=masquerade comment="AXbills Masquerade CUSTOM"};
      }
    }
...
=cut
    ...
    foreach my $tp_id ( sort keys %TARIF_SPEEDS ) {
      ...
      if ( $argv->{NAT} && !-f "$base_dir/libexec/mikrotik.nat" ) {
        ...
        push @commands,
          qq{/ip firewall nat add chain=srcnat action=$action $skip_nat_ips comment="AXbills Masquerade TP_$tp_id" src-address-list=CLIENTS_$tp_id $nat_ips };
      }
    }
    ...
    if ( $argv->{NAT} && -f "$base_dir/libexec/mikrotik.nat" ) {
      ...
      if ( open(my $fh, '<', "$base_dir/libexec/mikrotik.nat") ) {
        ...
        push @nat_cmd, qq{/ip firewall nat remove [find comment="AXbills Extended NAT"]};
        ...
        foreach my $line ( @rows ) {
          ...
          if ( $nas_id eq $nas_info->{nas_id} ) {
            ...
            push @nat_cmd,
              qq{/ip firewall nat add chain=srcnat action=netmap $skip_nat_ips $params to-addresses=$external_ip comment="AXbills Extended NAT" };
          }
        }
