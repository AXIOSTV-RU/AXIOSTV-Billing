...
sub _equipment_pon_load {
  ...
  my $SNMP_COMMUNITY = "$nas_info->{nas_mng_password}\@" . (($nas_info->{NAS_MNG_HOST_PORT}) ? $nas_info->{NAS_MNG_HOST_PORT} : $nas_info->{nas_ip});
  my $onu_counts = 0;
  ...
  if ($nas_info->{status} eq 0) {
    ...
    if (defined(&{$onu_list_fn})) {
      ...
      my $onu_snmp_list = &{\&$onu_list_fn}($olt_ports, {
        VERSION        => $nas_info->{snmp_version} || 1,
        SNMP_COMMUNITY => $SNMP_COMMUNITY,
        TIMEOUT        => $nas_info->{snmp_timeout} || 5,
        SKIP_TIMEOUT   => 1,
        DEBUG          => $debug,
        MODEL_NAME     => $nas_info->{model_name},
...
sub _scan_mac_serial_on_all_nas {
  ...
  foreach my $nas (@$Equipment_list) {
    ...
    my $SNMP_COMMUNITY = $nas->{NAS_MNG_PASSWORD} . '@' . $nas->{NAS_MNG_HOST_PORT};
...
#**********************************************************
=head2 _bdcom_add_descr() - Auto Add Descriptions to ONU interfaces
...
#Данные об OLT
  ...
  my $nas_type = $Equipment_list->[0]->{VENDOR_NAME};

#Данные об ONU
  my $onu_list = $Equipment->onu_list({
    NAS_ID        => ($attr->{onu_nas}) ? $attr->{onu_nas} : '_SHOW',
    ONU_DHCP_PORT => $attr->{onu_port},
    ONU_DESC      => '_SHOW',
    ONU_SNMP_ID   => '_SHOW',
    COLS_NAME     => 1
  });

  $Equipment_list->[0]->{snmp_index} = $onu_list->[0]->{onu_snmp_id};

#Адреса
  $Admin->query("SELECT u.id AS login,
if ((LENGTH(s.name) > 18), LEFT(s.name, 11), s.name) AS STREET,
b.number AS BUILDING,
up.address_flat AS KV
FROM users_pi up
INNER JOIN users u ON u.uid=up.uid
LEFT JOIN builds b ON b.id=up.location_id
LEFT JOIN streets s ON s.id=b.street_id
LEFT JOIN districts d ON d.id=s.district_id
WHERE up.uid = $attr->{uid}",
    undef,
    { COLS_NAME => 1 }
  );

  return [ ] if ($Admin->{errno});
  my $list = $Admin->{list}->[0];
...
#Транслитерация
  my $descr = $list->{STREET} . '_' . uc($list->{BUILDING})  . '_(' . $list->{login} . ')';
  $descr = ($list->{STREET} . '_' . uc($list->{BUILDING}) . '-' . $list->{KV} . '_(' . $list->{login} . ')') if ($list->{KV} && $list->{KV} =~ /^[0-9]+$/ && $list->{KV} > 0);

        $descr =~ s/А/A/g;              $descr =~ s/а/a/g;
        $descr =~ s/Б/B/g;              $descr =~ s/б/b/g;
        $descr =~ s/В/V/g;              $descr =~ s/в/v/g;
        $descr =~ s/Г/G/g;              $descr =~ s/г/g/g;
        $descr =~ s/Д/D/g;              $descr =~ s/д/d/g;
        $descr =~ s/Е/E/g;              $descr =~ s/е/e/g;
        $descr =~ s/Ё/E/g;              $descr =~ s/ё/e/g;
        $descr =~ s/Ж/Zh/g;             $descr =~ s/ж/zh/g;
        $descr =~ s/З/Z/g;              $descr =~ s/з/z/g;
        $descr =~ s/И/I/g;              $descr =~ s/и/i/g;
        $descr =~ s/Й/I/g;              $descr =~ s/й/i/g;
        $descr =~ s/К/K/g;              $descr =~ s/к/k/g;
        $descr =~ s/Л/L/g;              $descr =~ s/л/l/g;
        $descr =~ s/М/M/g;              $descr =~ s/м/m/g;
        $descr =~ s/Н/N/g;              $descr =~ s/н/n/g;
        $descr =~ s/О/O/g;              $descr =~ s/о/o/g;
        $descr =~ s/П/P/g;              $descr =~ s/п/p/g;
        $descr =~ s/Р/R/g;              $descr =~ s/р/r/g;
        $descr =~ s/ТС/TS/g;            $descr =~ s/Тс/Ts/g;    $descr =~ s/тс/ts/g;
        $descr =~ s/С/S/g;              $descr =~ s/с/s/g;
        $descr =~ s/Т/T/g;              $descr =~ s/т/t/g;
        $descr =~ s/У/U/g;              $descr =~ s/у/u/g;
        $descr =~ s/Ф/F/g;              $descr =~ s/ф/f/g;
        $descr =~ s/Х/X/g;              $descr =~ s/х/x/g;
        $descr =~ s/Ц/Ts/g;             $descr =~ s/ц/ts/g;
        $descr =~ s/Ч/Ch/g;             $descr =~ s/ч/ch/g;
        $descr =~ s/Ш/Sh/g;             $descr =~ s/ш/sh/g;
        $descr =~ s/Щ/Shch/g;           $descr =~ s/щ/shch/g;
        $descr =~ s/Ь/'/g;              $descr =~ s/ь/'/g;
        $descr =~ s/Ы/Y/g;              $descr =~ s/ы/y/g;
        $descr =~ s/Ъ/''/g;             $descr =~ s/ъ/''/g;
        $descr =~ s/Э/E/g;              $descr =~ s/э/e/g;
        $descr =~ s/Ю/Yu/g;             $descr =~ s/ю/yu/g;
        $descr =~ s/Я/Ya/g;             $descr =~ s/я/ya/g;
        $descr =~ s/ /-/g;

        my $set_desc_fn = lc('_' . $nas_type . '_add_descr');

        if (defined(&{$set_desc_fn}) && $conf{EQUIPMENT_SNMP_AUTO_DESC_WR}) {
            my $result = &{\&$set_desc_fn}($Equipment_list, $descr);
        }
...
#**********************************************************
=head2 _bdcom_add_descr() - Auto Add Descriptions to BDCOM ONU interfaces

=cut
#**********************************************************
sub _bdcom_add_descr {
  my ($attr, $descr) = @_;

  my $oid = '1.3.6.1.2.1.31.1.1.1.18.';

  my ($ip) = split(/:/, $attr->[0]->{NAS_MNG_HOST_PORT});
  my $SNMP_COMMUNITY = ($attr->[0]->{NAS_MNG_PASSWORD} || '') . '@' . $attr->[0]->{NAS_MNG_HOST_PORT};

  if ($attr->[0]->{snmp_index}) {
    $oid = $oid . $attr->[0]->{snmp_index};
  }

  if ($attr->[0]->{NAS_MNG_PASSWORD} && $attr->[0]->{NAS_MNG_HOST_PORT}) {
      my $result_get = snmp_get({
        OID            => $oid,
        SNMP_COMMUNITY => $SNMP_COMMUNITY,
	...
	SILENT         => 1
      ...
      if (defined($result_get) && $result_get ne $descr) {
        # print "Changing descr $result -> $descr ON OLT $attr->[0]->{NAS_ID} ($oid)\n";
         my $result = snmp_set({
            SNMP_COMMUNITY => $SNMP_COMMUNITY,
            SILENT         => 1,
            TIMEOUT        => $conf{EQUIPMENT_INFO_SNMP_TIMEOUT} || 10,
            RETRIES        => '10',
            OID            => [ $oid, "string", "$descr" ]
         });
	print "Changing descr $result_get -> $descr ON OLT $attr->[0]->{NAS_ID} ($oid -- $result)\n";
      }
  }
...
#**********************************************************
=head2 _bdcom_add_descr() - Auto Add Descriptions to C-DATA ONU interfaces

=cut
#**********************************************************
sub _cdata_add_descr {
  my ($attr, $descr) = @_;

  my $model = $attr->[0]->{MODEL_NAME} || '';

  my $oid = '1.3.6.1.4.1.34592.1.3.4.1.1.4.1.';
  $oid = '1.3.6.1.4.1.17409.2.3.4.1.1.2.' if ($model =~/^FD12/);
  ...
  if ($attr->[0]->{NAS_MNG_PASSWORD} && $attr->[0]->{NAS_MNG_HOST_PORT}) {
      my $result_get = snmp_get({
        OID            => $oid,
        SNMP_COMMUNITY => $SNMP_COMMUNITY,
        TIMEOUT        => $attr->[0]->{snmp_timeout} || 10
      });

      if (defined($result_get) && $oid && $result_get ne $descr) {
          #print "Changing descr $result -> $descr ON OLT $attr->[0]->{NAS_ID} ($oid)\n";
          my $result = snmp_set({
             SNMP_COMMUNITY => $SNMP_COMMUNITY,
             SILENT         => 1,
