...
sub form_nas_mng {
  ...
  if ($FORM{ext_info}) {
    ...
    if (-f "../wrt_configure.cgi") {
      ...
      $html->message('callout', 'Install wrt_configure.cgi', $html->button("AXbills Wiki ", '', { GLOBAL_URL => 'https://billing.axiostv.ru' }));
    }
  }
...
sub form_nas_list {
  ...
  my %ext_titles = (
    ...
    address_flat_alt => "$lang{ADDRESS} $lang{ADDRESS_FLAT_ALT}",
  ...
  result_former(
    {
      ...
      FUNCTION       => 'list',
      BASE_FIELDS    => 0,
      DEFAULT_FIELDS => 'NAS_ID,NAS_NAME,NAS_IDENTIFIER,NAS_IP,NAS_TYPE,DISABLE,ACCIDENT_CHECK,DESCR',
...
sub form_nas_add {
  ...
  $Nas->{NAS_ACCIDENT_CHECK} = ($Nas->{NAS_ACCIDENT_CHECK} && $Nas->{NAS_ACCIDENT_CHECK} > 0) ? ' checked' : '';
