...
sub form_user_info {
  ...
  $user_info->{COMPANY_NAME} = "$lang{NOT_EXIST}" if ($user_info->{IS_COMPANY} && !$user_info->{COMPANY_NAME});
...
sub user_pi {
  ...
  if ($FORM{SOCIAL_INFO}) {
    ...
    $FORM{IS_COMPANY} = $FORM{IS_COMPANY} ? 1 : 0;
    my %pi_form = %FORM;
    delete $pi_form{IS_COMPANY};

    $user->pi_add({ %pi_form });
    if (!$user->{errno} && $FORM{UID}) {
      $users->change($FORM{UID}, {
        UID                => $FORM{UID},
  ...
  $user_pi->{IS_COMPANY} = defined $FORM{IS_COMPANY} ? ($FORM{IS_COMPANY} ? 1 : 0) : ($user->{IS_COMPANY} || 0);
  $user_pi->{IS_COMPANY_CHECKED} = $user_pi->{IS_COMPANY} ? 'checked' : '';
  $user_pi->{COMPANY_NAME_DISPLAY} = $user_pi->{IS_COMPANY} ? '' : 'display:none';

  foreach my $field (qw/INN KPP OGRN OKPO BANK_BIC BANK_ACCOUNT COR_BANK_ACCOUNT/) {
    $user_pi->{$field} = '' if defined $user_pi->{$field} && $user_pi->{$field} eq '0';
  }
  ...
  if (!$attr->{QUICK_FORM} && ($permissions{0}{24} || ($FORM{index} && $FORM{index} != 15))) {
	...
	$FORM{DATE} ||= $user_pi->{CONTRACT_DATE};
	my $date_field = $html->form_datetimepicker('CONTRACT_DATE', $FORM{DATE}, {
        # FORM_ID => 'user_form',
        FORMAT  => 'YYYY-MM-DD HH:mm:ss'
	});
	
	$user_pi->{ADDON} = $html->form_input( 'hold_date', '1', {
        TYPE      => 'checkbox',
	...
	$user_pi->{DATE_FORM} = $html->tpl_show(templates('form_row_dynamic_size_input_group'), {
        ID    => 'DATE',
        NAME  => $lang{DATE} . ':',
        VALUE => $date_field,
...
sub form_users_list {
  ...
  ($table, $list) = result_former({
    ...
    FUNCTION        => 'list',
    BASE_FIELDS     => 1,
    DEFAULT_FIELDS  => "LOGIN,COMPANY_NAME,FIO,DEPOSIT,CREDIT,LOGIN_STATUS,$ext_fields",
    ...
    TABLE           => {
      ...
      MENU             => "$lang{ADD}:index=" . get_function_index('form_wizard') .
  ...
  my %user_list_col_attrs = (
    company_name => { style => 'width: 18%; min-width: 170px;' },
    fio          => { style => 'width: 22%; min-width: 220px;' },
  );
  ...
  foreach my $line (@$list) {
    ...
    for ($i = $base_fields; $i < $base_fields + $users->{SEARCH_FIELDS_COUNT}; $i++) {
      ...
      if ($col_name eq 'login_status') {
        ...
        push @fields_array, $table->td(
          ...
          { class => "text-$color", align => 'center', style => 'width: 90px; white-space: nowrap;' }
        );
      }
      else {
        push @fields_array, $table->td($line->{$col_name}, $user_list_col_attrs{$col_name});
      }
    }
    ...
    @fields_array = ($table->td(user_ext_menu($uid, $line->{login}, {
      ...
      deleted      => $line->{_del},
      NO_CHANGE    => 1
    }), { style => 'width: 14%; min-width: 170px; white-space: nowrap;' }), @fields_array);
...
sub users_import {
  ...
  $html->tpl_show(templates('form_import'), {
    IMPORT_FIELDS     => $conf{USER_IMPORT_FIELDS} || 'LOGIN,CONTRACT_ID,FIO,PHONE,ADDRESS_STREET,ADDRESS_BUILD,ADDRESS_FLAT,ADDRESS_FLAT_ALT,PASPORT_NUM,PASPORT_GRANT',
...
sub form_money_transfer_admin {
  ...
  if ($conf{MONEY_TRANSFER} && $conf{MONEY_TRANSFER} =~ /:/) {
    ...
    if ($no_companies eq 'NO_COMPANIES' && $user->{IS_COMPANY}) {
