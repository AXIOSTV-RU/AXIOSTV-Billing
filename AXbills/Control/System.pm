...
sub form_holidays {
  ...
  foreach my $line (@$list) {
    ...
    my $change = $html->button($lang{CHANGE}, "index=" . get_function_index('form_holidays') . "&change=$line->{day}", { class=>'change'});
    my $delete = $html->button($lang{DEL}, "index=" . get_function_index('form_holidays') . "&del=$line->{day}", { MESSAGE => "$lang{DEL} $line->{day}?", class => 'del' });
...
sub table_for_calendar{
  ...
  while ($i <= $attr->{MONTH_DAYS}) {
    ...
    if ($search && $holi_info->{DAY} && $search eq $holi_info->{DAY} ) {
      ...
      $action      = 'show';
      $delete = $html->button($lang{DEL}, "index=" . get_function_index('form_holidays') . "&del=".($attr->{MONTH})."-$i", { MESSAGE => "$lang{DEL} $attr->{MONTH}-$i?", class => 'del' });
    }
    ...
    $week_row .= "<td width='100' height='100' $holiday_day>
                  <a href='/admin/index.cgi?index=" . get_function_index('form_holidays') . "&$action=$attr->{MONTH}-$i&year=". ($FORM{year} || '') ."&month=". ($FORM{month} || '') ."' title='$comment'>
                    <h4>$i</h4><br>
                  </a>
                  $delete
                  </td>";
  ...
  if (defined($FORM{tt})) {
    ...
    $tarif_plan->{NETS_SEL} =  $html->form_select(
      'NET_ID',
      {
        ...
        MAIN_MENU      => get_function_index('internet_traffic_classes'),
        SEL_OPTIONS    => { '' => '--' },
    ...
    if(in_array('Internet', \@MODULES)) {
       if ($conf{RF_MODE}) {
        $html->tpl_show(_include('internet_tt_mod', 'Internet'), $tarif_plan);
       }
       else{
...
sub form_feedback {
  ...
  $html->tpl_show(templates('form_feedback'),{
    SYS_ID          => $sys_id,
    VERSION         => $version,
    HELP_DESK_LOGIN => $conf{HELP_DESK_LOGIN} || 'public_vers',
    pid             => $conf{HELP_DESK_PID} || 10000,
    issuetype       => $conf{HELP_DESK_ISSUETYPE} || 10000,
  });
