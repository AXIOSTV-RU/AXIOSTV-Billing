...
sub auth_admin {
  ...
  if ($conf{AUTH_METHOD}) {
    ...
    if (! $res) {
      if ($FORM{REFERER} && $FORM{REFERER} =~ /^\Q$SELF_URL\E/ && $FORM{REFERER} !~ /index=10/) {
  ...
  #**********************************************************
    ...
    if ($admin->{errno}) {
      ...
      if ($admin->{errno} == 2) {
	$admin->{errstr} = $lang{$admin->{errstr}} if ($lang{$admin->{errstr}});
...
sub form_login {
  ...
  if ($FORM{forgot_passwd} && $conf{ADMIN_PASSWORD_RECOVERY}) {
    if ($FORM{email}) {
      ...
      if ($admin->{TOTAL} && $admin->{TOTAL} > 0 && $admins_list && $admins_list->[0]->{aid}) {
        my $token = _admin_pw_recovery_token($admins_list->[0]->{aid}, $admins_list->[0]->{email} || $FORM{email});
        my $message = "Password recovery link (valid 1 hour):\n$SELF_URL?index=10&recovery_passwd=$token";
        sendmail("$conf{ADMIN_MAIL}", "$FORM{email}", "$PROGRAM Password Repair", "$message", "$conf{MAIL_CHARSET}", "");
      }
      $html->message('info', $lang{INFO} || 'Info', $lang{SENDED} || 'Message sent');
      exit;
    }
    else {
    ...
    my $admin_form = _admin_pw_recovery_check($FORM{recovery_passwd});
    if ($admin_form) {
      if ($FORM{newpassword}) {
        $admin_form->change({ PASSWORD => $FORM{newpassword}, AID => $admin_form->{AID} });
        if (!$admin_form->{errno}) {
        ...
        $password_form->{HIDDDEN_INPUT} = $html->form_input('recovery_passwd', $FORM{recovery_passwd}, { TYPE => 'hidden', OUTPUT2RETURN => 1 });
        $html->tpl_show(templates('form_password'), $password_form);
      }
    }
    else {
      $html->message('err', $lang{ERROR} || 'Error', $lang{ERR_WRONG_DATA} || 'Wrong data');
    }
    exit;
  }
...
sub check_permissions {
  ...
  if (($ENV{REQUEST_METHOD} || '') eq 'GET' && defined $FORM{passwd}) {
    $password = '';
  }
  ...
  if ($conf{ADMINS_ALLOW_IP}) {
    ...
    if (!$allow_ips_hash{ $ENV{REMOTE_ADDR} }) {
      $admin->system_action_add("$login:**** DENY IP: $ENV{REMOTE_ADDR}", { TYPE => 11 });
      $admin->{errno} = 3;
      return 3;
    }
  }
  ...
  if ($admin->{errno}) {
    if ($admin->{errno} == 4) {
      $admin->system_action_add("$login:****", { TYPE => 11 });
      $admin->{errno} = 4;
    }
    elsif ($admin->{errno} == 2) {
      return 2;
...
sub auth_user {
  ...
  if (($ENV{REQUEST_METHOD} || '') eq 'GET' && defined $FORM{passwd} && defined $password && $password eq $FORM{passwd}) {
    $password = '';
  }
...
#**********************************************************
=head2 _admin_pw_recovery_token($aid, $email, $exp)

  Signed recovery token. Valid for 1 hour.

=cut
#**********************************************************
sub _admin_pw_recovery_token {
  my ($aid, $email, $exp) = @_;
  ...
  require Digest::SHA;
  my $key = $conf{secretkey} || '';
  my $mac = Digest::SHA::hmac_sha256_hex("$aid|$email|$exp", $key);

  return "$aid.$exp.$mac";
}

#**********************************************************
=head2 _admin_pw_recovery_check($token)

=cut
#**********************************************************
sub _admin_pw_recovery_check {
  my ($token) = @_;

  return if (!$token || $token !~ /^(\d+)\.(\d+)\.([0-9a-f]{64})$/);

  my ($aid, $exp, $mac) = ($1, $2, $3);
  return if ($exp < time() || $exp > time() + 86400);

  my $admin_form = Admins->new($db, \%conf);
  $admin_form->info($aid);
  return if ($admin_form->{errno} || !$admin_form->{AID} || !$admin_form->{EMAIL});

  my $expect = _admin_pw_recovery_token($aid, $admin_form->{EMAIL}, $exp);
  return if ($expect ne $token);
