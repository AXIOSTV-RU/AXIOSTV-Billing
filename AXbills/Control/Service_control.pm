...
#	  ACTIONS   => '*->3',
	...
	if ($admin->{TOTAL} > 0) {
      ...
      foreach my $action (@$action_list) {
        ...
        elsif ($action->{action_type} == 8){
          my $period = date_diff($action->{date},$main::DATE);
          if($period < $hold_up_period){
            return { error => 4410, errstr => 'HOLDUP_MIN_PERIOD_NOT_EXPIRE' };
          }
        }
      }
    ### START AXbills ###
