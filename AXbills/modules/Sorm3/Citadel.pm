...
my $location_file = "$main::var_dir/sorm/Citadel";                                                                                                                                                                                   
...
sub new {
  ...
  $region_id = $self->{conf}->{CITADEL_ISP_ID} || $self->{conf}->{SORM3_ISP_ID} || q{};
  $zip = $self->{conf}->{OFFICE_ZIP} || q{};
  $country = $self->{conf}->{CITADEL_COUNTRY} || $self->{conf}->{SORM3_COUNTRY} || q{};
  $region = $self->{conf}->{CITADEL_REGION} || $self->{conf}->{SORM3_REGION} || q{};
  $zone = $self->{conf}->{CITADEL_ZONE} || $self->{conf}->{SORM3_ZONE} || q{};
  $city = $self->{conf}->{OFFICE_CITY} || q{};
  $street = $self->{conf}->{OFFICE_STREET} || q{};
  $build = $self->{conf}->{OFFICE_BUILD} || q{};
  $apart = $self->{conf}->{OFFICE_APART} || q{};
  $time_offset = $self->{conf}->{CITADEL_TIME_OFFSET} || $self->{conf}->{SORM3_TIME_OFFSET} || q{};
  $debug = $self->{argv}->{DEBUG} if($self->{argv}->{DEBUG});
  $Payments = Finance->payments($self->{db}, $self->{admin}, $self->{conf});
  $server_ip = $self->{conf}->{CITADEL_SERVER} || $self->{conf}->{SORM3_SERVER} || q{};
  $login = $self->{conf}->{CITADEL_LOGIN} || $self->{conf}->{SORM3_LOGIN} || q{};
  $password = $self->{conf}->{CITADEL_PASSWORD} || $self->{conf}->{SORM3_PASSWORD} || q{};
  $isp_description = $self->{conf}->{CITADEL_ISP_DESCRIPTION} || $self->{conf}->{SORM3_ISP_DESCRIPTION} || q{};
...
sub init() {
	...
	mkdir($main::var_dir . '/sorm/Citadel');
...
my $head_iden = join(';', @{ $self->{arr}->{ABONENT_IDEN} })."\n";
	...
	$OLD_ADDRESS = qq{
	    pi.city as REG_CITY,
	    pi.address_street as REG_STREET,
	    pi.address_build as REG_BUILDING,
	    pi.address_flat as REG_APARTMENT,
	    pi.city as CITY,
	...
	SELECT 
	    u.uid as ABONENT_ID,
	    u.uid as ACCOUNT,
	    if(pi.contract_date = '0000-00-00',concat_ws(' ', '1970-01-01', '00:00:00'),concat_ws(' ', pi.contract_date, '00:00:00')) as CONTRACT_DATE,
	    IF(pi.contract_id = '', u.uid, pi.contract_id) as CONTRACT,
	    if(pi.contract_date = '0000-00-00',concat_ws(' ', '1970-01-01', '00:00:00'),concat_ws(' ', pi.contract_date, '00:00:00')) as ACTUAL_FROM,
	...
	my $sorm_query = qq{
		SELECT ABONENT_ID,
			concat_ws(' ',CONTRACT_DATE, '00:00:00') as CONTRACT_DATE,
			CONTRACT,
			ACCOUNT,
			ABONENT_TYPE,
			UNSTRUCT_NAME,
	...
	foreach my $s (@sorm_data) {
		...
		if($s->{DATA}->{ACTUAL_TO} =~ /0000-00-00/) {
			...
			$sth = $db->prepare(qq{
			    DELETE FROM CITADEL_ABONENT_SERVICES WHERE ABONENT_ID='$s->{DATA}->{ABONENT_ID}';
			});
		...
		my($end, undef) = split(/ /, $s->{DATA}->{ACTUAL_TO});
    		...
    		if((date_diff($end, $current_date) > 1100) && $self->{conf}->{CITADEL_UPLOAD_USER_3_YEAR}) {
		        print date_diff($current_date, $end);
			my $sth = $db->prepare(qq{ DELETE FROM CITADEL_ABONENT WHERE ABONENT_ID = '$s->{DATA}->{ABONENT_ID}' });
			$sth->execute() or die DBI->errstr;
		}
		elsif(!$self->{conf}->{CITADEL_UPLOAD_USER_3_YEAR}) {
		    my $sth = $db->prepare(qq{ DELETE FROM CITADEL_ABONENT WHERE ABONENT_ID = '$s->{DATA}->{ABONENT_ID}' });
	    	    $sth->execute() or die DBI->errstr;
		}
	    }
	...
	foreach my $new_abonent (@new_abonent) {
		if($new_abonent) {
			...
			my $query_serv = qq{
			    INSERT INTO CITADEL_ABONENT_SERVICES(ABONENT_ID,ID,MODULE,BEGIN_TIME) VALUES(?,?,?,?)
			};
...
sub check_abonent_serv {
	...
	if($s->{SERVICES}&& exists $a->{SERVICES}) {
		...
		if($sorm_service->{ID} && exists $a->{SERVICES}->{ $sorm_service->{ID} }) {
		    ...
		    print ABONENT_SERV $self->mapper($sorm_service, 'ABONENT_SERV');
		    my $sth = $db->prepare(qq{ DELETE FROM CITADEL_ABONENT_SERVICES WHERE ID='$sorm_service->{ID}' AND ABONENT_ID='$sorm_service->{ABONENT_ID}' });
		    $sth->execute() or die DBI->errstr;
		}
	    }
	}
...
sub pay_types {
	...
	$type->{id} =~ s/^\s+|\s+$//g;
	$type->{id} //= 0;
	push @arr, "$type->{id};$global_begin;$date_ended;$type->{name};$self->{conf}->{CITADEL_ISP_ID}\n";
    }
...
sub sorm_payments {
    ...
    foreach my $entry (@sorm_pay) { 
		...
		my $sth = $dbh->prepare(qq{DELETE FROM CITADEL_PAYMENT WHERE axbills_id = '$entry->{axbills_id}' });
	...
	foreach my $new_entry (@new_pay) {
		...
		if($self->{argv}->{FULL_PAY}) {
			...
			if(date_diff($pay_date, $current_date) < 186) {
			    my $sth = $dbh->prepare(qq{
				    INSERT INTO CITADEL_PAYMENT ($add_query) 
		...
		#PAY 6 MONTH
		my $sth = $dbh->prepare(qq{ INSERT INTO CITADEL_PAYMENT ($add_query) 
                			    VALUES ('$new_entry->{PAYMENT_TYPE}', '$new_entry->{PAY_TYPE_ID}', '$new_entry->{PAYMENT_DATE}', 
                        			    '$new_entry->{AMOUNT}', '$new_entry->{ACCOUNT}',
                        			    '$new_entry->{axbills_id}', '$country','$region', 
						    '$zone', '$city', '$street', '$build', '$apart', 
...
sub send {                                                                     
  ...
  my $archive = $self->{conf}->{CITADEL_ARCHIVE} || '0';                          
  my $archive_path = $self->{conf}->{CITADEL_ARCHIVE_PATH} || '';

  my $ftp = Net::FTP->new($server_ip, Timeout => $self->{conf}->{CITADEL_FTP_TIMEOUT}, Debug => 0, Passive => $self->{conf}->{CITADEL_FTP_PASSIVE_MODE} || 0) or die "Cannot connect to $server_ip: $@"; 
  $ftp->login($login, $password) or die "Cannot login ", $ftp->message;
  $ftp->binary() if($self->{conf}->{CITADEL_FTP_BINNARY});
...
sub params_former {
	...
	NEW_PAYMENTS_ADD => [
	    ...
	    'CITY','STREET','BUILDING','APARTMENT','PHONE','ZIP'
	]
    };
...
print q{
------------------- HELP --------------------
/usr/axbills/billd citadel TYPE=Citadel [ATTR]=1
