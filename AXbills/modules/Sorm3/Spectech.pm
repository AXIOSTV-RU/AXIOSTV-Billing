...
my ($User, $Internet, $Sessions, $Nas, $Abon, $Admins, $Sorm, $debug);
...
sub _cfg {
  ...
  return $c->{"SORM3_$key"} if exists $c->{"SORM3_$key"} && defined $c->{"SORM3_$key"};
  return $c->{"SPECTECH_$key"} if exists $c->{"SPECTECH_$key"} && defined $c->{"SPECTECH_$key"};
  return;
}
...
sub _reports_init {
  ...
  my $dir = Sorm3::Sorm::export_dir($self->{conf});
  my $base = "$main::var_dir/sorm/$dir";

  %reports = (
    ABONENT               => "$base/ABONENT_" . $sufix,
    ABONENT_IDENT         => "$base/ABONENT_IDENT_" . $sufix,
    ABONENT_SRV           => "$base/ABONENT_SRV_" . $sufix,
    ABONENT_ADDR          => "$base/ABONENT_ADDR_" . $sufix,
    PAYMENT               => "$base/PAYMENT_" . $sufix,
    PAY_TYPE              => "$base/PAY_TYPE_" . $sufix,
    DOC_TYPE              => "$base/DOC_TYPE_" . $sufix,
    IP_PLAN               => "$base/IP_PLAN_" . $sufix,
    GATEWAY               => "$base/GATEWAY_" . $sufix,
    IP_GATEWAY            => "$base/IP_GATEWAY_" . $sufix,
    SUPPLEMENTARY_SERVICE => "$base/SUPPLEMENTARY_SERVICE_" . $sufix,
    REGIONS               => "$base/REGIONS_" . $sufix,
  );
...
sub init {
  ...
  my $export_dir = $self->_reports_init();
  $Sorm = Sorm3::Sorm->new($self->{db}, $self->{admin}, $self->{conf});
  ...
  if ($argv->{START}) {
    ...
    mkdir($main::var_dir . '/sorm/' . $export_dir);
...
sub static_report {
  ...
  if (! -f "$main::var_dir/sorm/Spectech/static/$type" ) {
    return 0;
  }

  my $content = q{};
  open(my $fh, '<', "$main::var_dir/sorm/Spectech/static/$type");
    $content = <$fh>;
  close($fh);
...
sub send {
  ...
  my $archive = $self->_cfg('ARCHIVE') || '0';
  my $archive_path = $self->_cfg('ARCHIVE_PATH') || '';
