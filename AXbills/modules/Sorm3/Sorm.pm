...
=head1 NAME
  ...
  Shared Spectech/Norsi SORM export (Sorm3)
...
my $date_ended = '2032-01-01 00:00:00';
my $debug = '0';

# $conf{SORM3_TYPE}         = spectech | norsi
# $conf{SORM3_TABLE_PREFIX} = SPECTECH | NORSI (optional)
# $conf{SORM3_EXPORT_DIR}   = Spectech | Norsi (subdir under var/sorm)
my $sorm_pfx = 'SPECTECH';
my $sorm_default_zip = '398001';
my $location_file = "$main::var_dir/sorm/Spectech/";
...
#**********************************************************
sub table_prefix {
  return Sorm3::Vendor::table_prefix($_[0]);
}
...
#**********************************************************
sub export_dir {
  return Sorm3::Vendor::export_dir($_[0]);
}

#**********************************************************
=head2 _sorm3_conf($key, $default)
...
sub _sorm3_conf {
  ...
  return $conf->{"SORM3_$key"} if exists $conf->{"SORM3_$key"} && defined $conf->{"SORM3_$key"};
  return $conf->{"${sorm_pfx}_$key"} if exists $conf->{"${sorm_pfx}_$key"} && defined $conf->{"${sorm_pfx}_$key"};
  return $default;
}
...
sub _bind_vendor {
  ...
  $sorm_pfx = table_prefix($self->{conf});
  $location_file = "$main::var_dir/sorm/" . export_dir($self->{conf}) . "/";
  $sorm_default_zip = $self->_sorm3_conf('DEFAULT_ZIP', '398001');
...
#**********************************************************
=head2 _abonent_detach_date($attr)

=cut
#**********************************************************
sub _abonent_detach_date {
  my ($attr) = @_;

  my $expire = $attr->{INTERNET_EXPIRE} || q{};
  return $date_ended if ($attr->{STATUS} || 0) ne '1';
  return $date_ended if !$expire || $expire =~ /^0000-00-00/;

  return $expire =~ /^\d{4}-\d{2}-\d{2}$/ ? "$expire 00:00:00" : $expire;
}

#**********************************************************
=head2 _parse_abonent_address($comments)

=cut
#**********************************************************
sub _parse_abonent_address {
  my ($comments) = @_;

  my %address = (
    ZIP       => $sorm_default_zip,
    CITY      => q{},
    STREET    => q{},
    BUILDING  => q{},
    APARTMENT => q{},
  ...
  my ($line) = split(/\r/, $comments || q{}, 2);
  $line //= q{};
  $line =~ s/\n.*$//s;
  $line =~ s/^\s+|\s+$//g;

  if ($line =~ /^\s*\S+\.\s+([^,\s]+),?\s+\S+\.\s+(.+?)\s*,?\s+(?:д|вл|уч)\.\s+([^,\s]+)(?:\s*,?\s+\S+\.\s+([^,\s]+))?/i ) {
    $address{CITY} = $1 || q{};
    $address{STREET} = $2 || q{};
    $address{BUILDING} = $3 || q{};
    $address{APARTMENT} = $4 || q{};

    for my $field (qw/CITY STREET BUILDING APARTMENT/) {
      $address{$field} =~ s/^\s+|\s+$//g;
    }
  }
...
#**********************************************************
=head2 _print_service_abonent_ident($fh, $attr)

=cut
#**********************************************************
sub _print_service_abonent_ident {
  my ($fh, $attr) = @_;

  my $pools = $attr->{POOLS} || [];
  return 1 if !@{$pools};

  my $base_abonent = $attr->{BASE_ABONENT} || die "Spectech service ABONENT_IDENT base UID=1 not found";
  my $region_id = $attr->{REGION_ID} || q{};

  my $phone = $base_abonent->{PHONE} || q{};
  my $email = $base_abonent->{E_MAIL} || q{};
  my $begin_time = $base_abonent->{CONTRACT_DATE} || q{};
  ...
  foreach my $pool (@{$pools}) {
    my $ipv4_start = $pool->[1];
    my $ipv4_end = $pool->[2];
    next if !$ipv4_start || !$ipv4_end;

    foreach my $cidr (Net::CIDR::range2cidr("$ipv4_start - $ipv4_end")) {
      my $to_hex = new Net::IP($cidr);
      my @hexip = split(/x/, $to_hex->hexip());
      my $clean_hex_ip = uc($hexip[1] || q{});

      while (length($clean_hex_ip) < 8) {
        $clean_hex_ip = "0$clean_hex_ip";
      }

      my @hexmask = split(/x/, $to_hex->hexmask());
      my $clean_hex_mask = uc($hexmask[1] || q{});

      print {$fh} "$base_abonent->{ABONENT_ID};$region_id;5;$phone;;;;;;;0;;;;;$email;;;;;0;$clean_hex_ip;;$clean_hex_mask;;$begin_time;$date_ended;;;;;;;;;;;;;;;1;;\n";
    }
  }
...
sub pool_sync {
  ...
  my $self = shift;
    $self->query2("UPDATE ${sorm_pfx}_IP_PLAN SET actual = '0', END_TIME = CURRENT_TIMESTAMP()");
    $self->query2("INSERT INTO ${sorm_pfx}_IP_PLAN (DESCRIPTION, IPV4_START, IPV4_END)
                SELECT name, INET_NTOA(ip), INET_NTOA(ip+counts-1) FROM ippools GROUP BY ip");

    foreach my $pool (@{ $self->_sorm3_conf('BACKBON_IP_PLAN') || [] }) {
      my ($description, $ipv4_start, $ipv4_end) = @{$pool};

      $self->query2("INSERT INTO ${sorm_pfx}_IP_PLAN (DESCRIPTION, IPV4_START, IPV4_END)
                  VALUES (?, ?, ?)",
                  'do',
                  { Bind => [ $description, $ipv4_start, $ipv4_end ] });
    }
...
sub abonentGetNewData
{
	...
	my $region_id = $self->_sorm3_conf('ISP_ID', q{});
	my $country = $self->_sorm3_conf('COUNTRY', q{});
	my $region = $self->_sorm3_conf('REGION', q{});
	my $zone = $self->_sorm3_conf('ZONE', q{});
	my $time_offset = $self->_sorm3_conf('TIME_OFFSET', '-3'); #Moscow

	my DBI $dbh = $self->{db}->{db};

	my $sth = $dbh->prepare(qq{SELECT users.uid AS ABONENT_ID,
users.is_company AS IS_COMPANY,
IF(users_pi.contract_date != '0000-00-00 00:00:00', users_pi.contract_date, users.registration) AS CONTRACT_DATE,
IF(users_pi.contract_id = '', users.id, users_pi.contract_id) AS CONTRACT_ID,
users.id AS ACCOUNT,
if((users.is_company = true), 43, 42) AS ABONENT_TYPE,
if((users.is_company = true), users_pi.company_name, '') AS FULL_NAME,
if((users.is_company = true), users_pi.inn, '') AS INN,
if ((fio2 != '' && fio3 != ''), CONCAT(fio, ' ', fio2, ' ', fio3), fio) AS CONTACT,
'' AS PHONE_FAX,
if((users.is_company = true), users_pi.bank_name, '') AS BANK,
if((users.is_company = true), users_pi.bank_account, '') AS BANK_ACCOUNT,
if ((fio2 != '' && fio3 != ''), CONCAT(fio, ' ', fio2, ' ', fio3), fio) AS UNSTRUCT_NAME,
if((users_pi.birth_date = '0000-00-00'), '', users_pi.birth_date) AS BIRTH_DATE,
if((users_pi.pasport_num != ''), CONCAT(users_pi.pasport_num, ' '), '') AS IDENT_CARD_SERIAL,
if((users_pi.pasport_grant != '' && users_pi.pasport_date != '0000-00-00'), concat(users_pi.pasport_grant, ' ', users_pi.pasport_date), '') AS IDENT_CARD_DESCRIPTION,
CASE
    ...
    ELSE '0'
END AS STATUS,
internet_main.expire AS INTERNET_EXPIRE,
#Abonent_addr
users_pi.comments AS ADDRESS_COMMENTS,
...
tarif_plans.id AS TP_ID,
#Abonent_ident
#users.id AS LOGIN,
if((internet_main.cid != 'ANY'), internet_main.cid, users.id) AS LOGIN,
(SELECT value from users_contacts WHERE type_id=2 AND users_contacts.uid=users.uid LIMIT 1) AS PHONE,
...
if ((users.is_company = true), DATE_ADD(users_pi.last_update, INTERVAL $time_offset HOUR), '0000-00-00 00:00:00') AS companies_updated
from users_pi
...
#LEFT JOIN builds b ON b.id=users_pi.location_id
...
$sth->execute or die DBI->errstr;
	...
	{
		my $address = _parse_abonent_address($new_data->{ADDRESS_COMMENTS});
		$new_data->{ZIP} = $address->{ZIP};
		$new_data->{CITY} = $address->{CITY};
		$new_data->{STREET} = $address->{STREET};
		$new_data->{BUILDING} = $address->{BUILDING};
		$new_data->{APARTMENT} = $address->{APARTMENT};
		$new_data->{DETACH} = _abonent_detach_date($new_data);
...
#Соберем в массив данные из базы скрипта
	$sth = $dbh->prepare(qq{SELECT ABONENT_ID,
		...
		companies_updated
		from ${sorm_pfx}_ABONENT
		ORDER BY ABONENT_ID ASC});
#if((IS_COMPANY = true),companies_updated, '0000-00-00 00:00:00') AS companies_updated
        $sth->execute or die DBI->errstr;
	...
	my $is_initial_abonent_export = scalar(@db_array) == 0;
		...
		$need_update_db = 0;
			 ...
			 if ($axbills_array{$entry->{id}}->{ABONENT_ID} eq $entry->{value}->{ABONENT_ID})
				...
				$begin_time = $entry->{value}->{CURRENT_TIMESTAMP_3} if ($begin_time =~ '0000-00-00 00:00:00');
                                ...
                                $axbills_array{$entry->{id}}->{LOGIN} //= q{};$axbills_array{$entry->{id}}->{LOGIN} =~ s/[;)(]/ /g;
                                $axbills_array{$entry->{id}}->{E_MAIL} //= q{};$axbills_array{$entry->{id}}->{E_MAIL} =~ s/[;)(]/ /g;
                                $axbills_array{$entry->{id}}->{DETACH} //= $date_ended;$axbills_array{$entry->{id}}->{DETACH} =~ s/[;)(]/ /g;
				...
				defined($axbills_array{$entry->{id}}->{DETACH}) && defined($entry->{value}->{DETACH}) && $axbills_array{$entry->{id}}->{DETACH} ne $entry->{value}->{DETACH} ||
				...
				defined($axbills_array{$entry->{id}}->{CONTACT}) && defined($entry->{value}->{CONTACT}) && $axbills_array{$entry->{id}}->{CONTACT} ne $entry->{value}->{CONTACT} ||
                                defined($axbills_array{$entry->{id}}->{IS_COMPANY}) && defined($entry->{value}->{IS_COMPANY}) && $axbills_array{$entry->{id}}->{IS_COMPANY} ne $entry->{value}->{IS_COMPANY} ||
...
#                                print Abonent "$entry->{value}->{ABONENT_ID};$region_id;$axbills_array{$entry->{id}}->{CONTRACT_DATE};$axbills_array{$entry->{id}}->{CONTRACT_ID};$axbills_array{$entry->{id}}->{ACCOUNT};$begin_time;$date_ended;$axbills_array{$entry->{id}}->{ABONENT_TYPE};1;;;;$axbills_array{$entry->{id}}->{UNSTRUCT_NAME};$axbills_array{$entry->{id}}->{BIRTH_DATE};0;1;;;;$axbills_array{$entry->{id}}->{IDENT_CARD_SERIAL}$axbills_array{$entry->{id}}->{IDENT_CARD_DESCRIPTION};$axbills_array{$entry->{id}}->{BANK};$axbills_array{$entry->{id}}->{BANK_ACCOUNT};$axbills_array{$entry->{id}}->{FULL_NAME};$axbills_array{$entry->{id}}->{INN};$axbills_array{$entry->{id}}->{CONTACT};$axbills_array{$entry->{id}}->{PHONE_FAX};$axbills_array{$entry->{id}}->{STATUS};$begin_time;$date_ended;4;1;\n";
                                print Abonent "$entry->{value}->{ABONENT_ID};$region_id;$axbills_array{$entry->{id}}->{CONTRACT_DATE};$axbills_array{$entry->{id}}->{CONTRACT_ID};$axbills_array{$entry->{id}}->{ACCOUNT};$begin_time;$date_ended;$axbills_array{$entry->{id}}->{ABONENT_TYPE};1;;;;$axbills_array{$entry->{id}}->{UNSTRUCT_NAME};$axbills_array{$entry->{id}}->{BIRTH_DATE};0;1;;;;$axbills_array{$entry->{id}}->{IDENT_CARD_SERIAL}$axbills_array{$entry->{id}}->{IDENT_CARD_DESCRIPTION};$axbills_array{$entry->{id}}->{BANK};$axbills_array{$entry->{id}}->{BANK_ACCOUNT};$axbills_array{$entry->{id}}->{FULL_NAME};$axbills_array{$entry->{id}}->{INN};$axbills_array{$entry->{id}}->{CONTACT};$axbills_array{$entry->{id}}->{PHONE_FAX};$axbills_array{$entry->{id}}->{STATUS};$begin_time;$axbills_array{$entry->{id}}->{DETACH};4;2;\n";

                                        my $sth = $dbh->prepare(qq{
                                                UPDATE ${sorm_pfx}_ABONENT SET
							...
							IS_COMPANY = '$axbills_array{$entry->{id}}->{IS_COMPANY}',
                                                        STATUS = '$axbills_array{$entry->{id}}->{STATUS}',
							ATTACH = '$begin_time',
							DETACH = '$axbills_array{$entry->{id}}->{DETACH}',
				...
				defined($axbills_array{$entry->{id}}->{LOGIN}) && defined($entry->{value}->{LOGIN}) && $axbills_array{$entry->{id}}->{LOGIN} ne $entry->{value}->{LOGIN} ||
		...
		#			print "IDENT Changed ON $entry->{id}";
				#Удаление строки.Как старая, но с RECORD_ACTON=2.
				print Abonent_ident "$entry->{value}->{ABONENT_ID};$region_id;5;$entry->{value}->{PHONE};;;;;;;0;;;;$entry->{value}->{LOGIN};$entry->{value}->{E_MAIL};;;;;;$clean_hex_ip_old;;FFFFFFFF;;$entry->{value}->{ABONENT_IDENT_BEGIN_TIME};$entry->{value}->{ABONENT_IDENT_END_TIME};;;;;;;;;;;;;;;2;;\n";

				#Запись копии старой строки, но с RECORD_ACTION=1 и END_TIME=$begin_time.
#				print Abonent_ident "$entry->{value}->{ABONENT_ID};$region_id;5;$entry->{value}->{PHONE};;;;;;;0;;;;$entry->{value}->{LOGIN};$entry->{value}->{E_MAIL};;;;;;$clean_hex_ip_old;;FFFFFFFF;;$entry->{value}->{ABONENT_IDENT_BEGIN_TIME};$begin_time;;;;;;;;;;;;;;;1;;\n";

				#Запись новой строки, с RECORD_ACTION=1 и BEGIN_TIME=$begin_time, END_TIME=$date_ended
				print Abonent_ident "$entry->{value}->{ABONENT_ID};$region_id;5;$axbills_array{$entry->{id}}->{PHONE};;;;;;;0;;;;$axbills_array{$entry->{id}}->{LOGIN};$axbills_array{$entry->{id}}->{E_MAIL};;;;;;$clean_hex_ip_new;;FFFFFFFF;;$begin_time;$date_ended;;;;;;;;;;;;;;;1;;\n";
...
#                        print Abonent_ident "$entry->{value}->{ABONENT_ID};$region_id;5;$entry->{value}->{PHONE};;;;;;;0;;;;$entry->{value}->{LOGIN};$entry->{value}->{E_MAIL};;;;;;$clean_hex_ip_old;;FFFFFFFF;;$begin_time;$date_ended;;;;;;;;;;;;;;;2;;\n";
                        print Abonent_ident "$entry->{value}->{ABONENT_ID};$region_id;5;$entry->{value}->{PHONE};;;;;;;0;;;;$entry->{value}->{LOGIN};$entry->{value}->{E_MAIL};;;;;;$clean_hex_ip_old;;FFFFFFFF;;$begin_time;$entry->{value}->{CURRENT_TIMESTAMP_3};;;;;;;;;;;;;;;1;;\n";

                        my $sth = $dbh->prepare(qq{DELETE FROM ${sorm_pfx}_ABONENT WHERE ABONENT_ID = '$entry->{id}';
                        });
...
#Новый абонент
		...
		if ($new_entry)
		{
                                ...
                                $new_entry->{DETACH} //= $date_ended;$new_entry->{DETACH} =~ s/[;)(]/ /g;
                        if ($abonent_created == 0)
                        {
                        open(Abonent, ">$location_file/ABONENT_$file.txt") || die;
                        print Abonent "ID;REGION_ID;CONTRACT_DATE;CONTRACT;ACCOUNT;ACTUAL_FROM;ACTUAL_TO;ABONENT_TYPE;NAME_INFO_TYPE;FAMILY_NAME;GIVEN_NAME;INITIAL_NAME;UNSTRUCT_NAME;BIRTH_DATE;IDENT_CARD_TYPE_ID;IDENT_CARD_TYPE;IDENT_CARD_SERIAL;IDENT_CARD_NUMBER;IDENT_CARD_DESCRIPTION;IDENT_CARD_UNSTRUCT;BANK;BANK_ACCOUNT;FULL_NAME;INN;CONTACT;PHONE_FAX;STATUS;ATTACH;DETACH;NETWORK_TYPE;RECORD_ACTION;INTERNAL_ID1\n";
                        ...
                        open(Abonent_ident, ">$location_file/ABONENT_IDENT_$file.txt") || die;
                        print Abonent_ident "ABONENT_ID;REGION_ID;IDENT_TYPE;PHONE;INTERNAL_NUMBER;IMSI;IMEI;ICC;MIN;ESN;EQUIPMENT_TYPE;MAC;VPI;VCI;LOGIN;E_MAIL;PIN;USER_DOMAIN;RESERVED;ORIGINATOR_NAME;IP_TYPE;IPV4;IPV6;IPV4_MASK;IPV6_MASK;BEGIN_TIME;END_TIME;LINE_OBJECT;LINE_CROSS;LINE_BLOCK;LINE_PAIR;LINE_RESERVED;LOC_TYPE;LOC_LAC;LOC_CELL;LOC_TA;LOC_CELL_WIRELESS;LOC_MAC;LOC_LATITUDE;LOC_LONGITUDE;LOC_PROJECTION_TYPE;RECORD_ACTION;INTERNAL_ID1;INTERNAL_ID2\n";
                        _print_service_abonent_ident(\*Abonent_ident, {
                          BASE_ABONENT => $axbills_array{1},
                          POOLS        => $self->_sorm3_conf('BACKBON_IP_PLAN') || [],
                          REGION_ID    => $region_id,
                        }) if ($is_initial_abonent_export);
                        $abonent_ident_created = 1;
                        }

			print Abonent "$new_entry->{ABONENT_ID};$region_id;$new_entry->{CONTRACT_DATE};$new_entry->{CONTRACT_ID};$new_entry->{ACCOUNT};$new_entry->{CONTRACT_DATE};$date_ended;$new_entry->{ABONENT_TYPE};1;;;;$new_entry->{UNSTRUCT_NAME};$new_entry->{BIRTH_DATE};0;1;;;;$new_entry->{IDENT_CARD_SERIAL}$new_entry->{IDENT_CARD_DESCRIPTION};$new_entry->{BANK};$new_entry->{BANK_ACCOUNT};$new_entry->{FULL_NAME};$new_entry->{INN};$new_entry->{CONTACT};$new_entry->{PHONE_FAX};$new_entry->{STATUS};$new_entry->{CONTRACT_DATE};$new_entry->{DETACH};4;1;\n";
			print Abonent_addr "$new_entry->{ABONENT_ID};$region_id;0;0;$new_entry->{ZIP};$country;$region;$zone;$new_entry->{CITY};$new_entry->{STREET};$new_entry->{BUILDING};;$new_entry->{APARTMENT};;$new_entry->{CONTRACT_DATE};$date_ended;1;;\n";
			print Abonent_addr "$new_entry->{ABONENT_ID};$region_id;3;0;$new_entry->{ZIP};$country;$region;$zone;$new_entry->{CITY};$new_entry->{STREET};$new_entry->{BUILDING};;$new_entry->{APARTMENT};;$new_entry->{CONTRACT_DATE};$date_ended;1;;\n";
			print Abonent_srv "$new_entry->{ABONENT_ID};$region_id;$new_entry->{TP_ID};$new_entry->{CONTRACT_DATE};$date_ended;$new_entry->{PARAMETER};$new_entry->{CONTRACT_ID};1;;\n";
			...
			print Abonent_ident "$new_entry->{ABONENT_ID};$region_id;5;$new_entry->{PHONE};;;;;;;0;;;;$new_entry->{LOGIN};$new_entry->{E_MAIL};;;;;;$clean_hex_ip_new;;FFFFFFFF;;$new_entry->{CONTRACT_DATE};$date_ended;;;;;;;;;;;;;;;1;;\n";
			#print Dumper \$new_entry;

			my $sth = $dbh->prepare(qq{INSERT INTO ${sorm_pfx}_ABONENT (ABONENT_ID, CONTRACT_DATE, CONTRACT, ACCOUNT, ABONENT_TYPE, UNSTRUCT_NAME, BIRTH_DATE,
        	        IDENT_CARD_TYPE_ID, IDENT_CARD_SERIAL, IDENT_CARD_DESCRIPTION, IS_COMPANY, BANK, BANK_ACCOUNT, FULL_NAME, INN, CONTACT, PHONE_FAX,
			...
			users_pi_updated, users_updated, internet_main_updated, companies_updated)
				...
				'0', '$new_entry->{IDENT_CARD_SERIAL}', '$new_entry->{IDENT_CARD_DESCRIPTION}', '$new_entry->{IS_COMPANY}', '$new_entry->{BANK}', 
				'$new_entry->{BANK_ACCOUNT}', 
				'$new_entry->{FULL_NAME}', '$new_entry->{INN}', '$new_entry->{CONTACT}', '$new_entry->{PHONE_FAX}', 
				'$new_entry->{STATUS}', 
				'$new_entry->{CONTRACT_DATE}', '$new_entry->{DETACH}', '4', '$new_entry->{CONTRACT_DATE}', '$date_ended', 
				'$new_entry->{ZIP}', '$country', '$region', 
				'$zone', '$new_entry->{CITY}', 
				'$new_entry->{STREET}', '$new_entry->{BUILDING}', '$new_entry->{APARTMENT}', 
				'$new_entry->{CONTRACT_DATE}', 
				'$date_ended', '$new_entry->{TP_ID}',
                                '$new_entry->{CONTRACT_DATE}','$date_ended', '$new_entry->{PARAMETER}', '$new_entry->{PHONE}', '0', 
				'$new_entry->{LOGIN}', '$new_entry->{E_MAIL}', '0', '$new_entry->{IPV4}', '$new_entry->{IPV4_MASK}', 
				'$new_entry->{CONTRACT_DATE}', '$date_ended', '$new_entry->{users_pi_updated}', 
	...
	if ($abonent_created)       { close(Abonent)       || die; }
	if ($abonent_addr_created)  { close(Abonent_addr)  || die; }
	if ($abonent_srv_created)   { close(Abonent_srv)   || die; }
	if ($abonent_ident_created) { close(Abonent_ident) || die; }
}
...
#Платежи из базы скрипта
	...
	printf "======PAYMENTS SECTION=======\n";
        ...
        my $sth = $dbh->prepare(qq{SELECT PAYMENT_TYPE, PAY_TYPE_ID, PAYMENT_DATE, AMOUNT, AMOUNT_CURRENCY, ACCOUNT,
                                ${sorm_pfx}_PAYMENT.ABONENT_ID AS ABONENT_ID, ${sorm_pfx}_PAYMENT.ZIP, ${sorm_pfx}_PAYMENT.COUNTRY, ${sorm_pfx}_PAYMENT.REGION, ${sorm_pfx}_PAYMENT.ZONE, ${sorm_pfx}_PAYMENT.STREET, 
				${sorm_pfx}_PAYMENT.BUILDING, ${sorm_pfx}_PAYMENT.APARTMENT, axbills_id, ${sorm_pfx}_PAYMENT.PHONE
                                FROM ${sorm_pfx}_PAYMENT
				LEFT JOIN ${sorm_pfx}_ABONENT ON ${sorm_pfx}_PAYMENT.ABONENT_ID = ${sorm_pfx}_ABONENT.ABONENT_ID
                                ORDER BY ${sorm_pfx}_PAYMENT.axbills_id ASC
				});
        $sth->execute or die DBI->errstr;
...
#Платежи из Абиллса
		...
		users_pi.zip AS ZIP
		FROM payments
		INNER JOIN users ON users.uid = payments.uid
...
##        	        print Payments "$region_id;83;$entry->{PAY_TYPE_ID};$entry->{PAYMENT_DATE};$entry->{AMOUNT};;$entry->{PHONE};$entry->{ACCOUNT};$entry->{ABONENT_ID};;;;;;;;;0;;;;;;;;;;0;0;$entry->{ZIP};$entry->{COUNTRY};$entry->{REGION};$entry->{ZONE};$entry->{CITY};$entry->{STREET};$entry->{BUILDING};;$entry->{APARTMENT};;2\n";
        	        print Payments "$region_id;83;$entry->{PAY_TYPE_ID};$entry->{PAYMENT_DATE};$entry->{AMOUNT};;$entry->{PHONE};$entry->{ACCOUNT};$entry->{ABONENT_ID};;;;;;;;;0;;;;;;;;;;0;0;$zip;$entry->{COUNTRY};$entry->{REGION};$entry->{ZONE};$entry->{CITY};$entry->{STREET};$entry->{BUILDING};;$entry->{APARTMENT};;2\n";

                        my $sth = $dbh->prepare(qq{DELETE FROM ${sorm_pfx}_PAYMENT WHERE axbills_id = '$entry->{axbills_id}'
                        });
                        $sth->execute() if ($debug == '0');
...
#
#                        my $sth = $dbh->prepare(qq{INSERT INTO ${sorm_pfx}_PAYMENT (PAYMENT_TYPE, PAY_TYPE_ID, PAYMENT_DATE, AMOUNT, ABONENT_ID, axbills_id, COUNTRY, REGION,
...
foreach my $new_entry (@new_p_entries) {
    if (defined($new_entry->{axbills_id}) && $new_entry->{AMOUNT} > '0') {
        ...
        # Проверка и инициализация значений
        my $pay_type_id = defined($new_entry->{PAY_TYPE_ID}) ? $new_entry->{PAY_TYPE_ID} : '';
        my $payment_date = defined($new_entry->{PAYMENT_DATE}) ? $new_entry->{PAYMENT_DATE} : '';
        my $amount = defined($new_entry->{AMOUNT}) ? $new_entry->{AMOUNT} : '';
        my $phone = defined($new_entry->{PHONE}) ? $new_entry->{PHONE} : ''; # Устанавливаем пустую строку, если PHONE не задан
        my $account = defined($new_entry->{ACCOUNT}) ? $new_entry->{ACCOUNT} : '';
        my $abonent_id = defined($new_entry->{ABONENT_ID}) ? $new_entry->{ABONENT_ID} : '';
#        my $zip = defined($new_entry->{ZIP}) ? $new_entry->{ZIP} : '';

        print Payments "$region_id;83;$pay_type_id;$payment_date;$amount;;$phone;$account;$abonent_id;;;;;;;;;0;;;;;;;;;;0;0;$zip;$country;$region;$zone;$city;$street;$build;;$apart;;1\n";

        # Параметризованный запрос
        my $sth = $dbh->prepare(qq{
            INSERT INTO ${sorm_pfx}_PAYMENT (PAYMENT_TYPE, PAY_TYPE_ID, PAYMENT_DATE, AMOUNT, ABONENT_ID, axbills_id, COUNTRY, REGION,
                ZONE, CITY, STREET, BUILDING, APARTMENT, PHONE, ZIP) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        });

        # Выполнение запроса
        $sth->execute('83', $pay_type_id, $payment_date, $amount, $abonent_id, 
                       $new_entry->{axbills_id}, $country, $region, $zone, $city, 
                       $street, $build, $apart, $phone, $zip) if ($debug == '0');
    }
}
...
#####
	...
	if ($file_created) { close(Payments) || die; }
...
#Export to Files
	 my @cidr_list;
         ...
         my $sth = $dbh->prepare(qq{SELECT id, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL $time_offset HOUR) AS CURRENT, DESCRIPTION, BEGIN_TIME, END_TIME, IPV4_START, IPV4_END, IP_TYPE 
					FROM ${sorm_pfx}_IP_PLAN WHERE reported = '0' ORDER BY id ASC});
         $sth->execute or die DBI->errstr;
	 ...
	 $sth = $dbh->prepare(qq{UPDATE ${sorm_pfx}_IP_PLAN SET reported = '1'});
         $sth->execute or die DBI->errstr;
...
#Из базы скрипта
	 ...
	 my DBI $dbh = $self->{db}->{db};
         ...
         my $sth = $dbh->prepare(qq{SELECT record_id, DISABLE, GATE_ID AS id, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL $time_offset HOUR) AS CURRENT, BEGIN_TIME, END_TIME, 
					...
					FROM ${sorm_pfx}_GATEWAY WHERE ended = '0' ORDER BY record_id DESC});
...
#Ищем совпадения, выпиливаем их
                ...
                if ($entry->{id} && !exists $axbills_array{$entry->{id}} && $entry->{END_TIME} eq $date_ended)
		{
			$need_update_files = 1;
                        #Deleting
			my $sth = $dbh->prepare(qq{UPDATE ${sorm_pfx}_GATEWAY SET END_TIME = '$entry->{CURRENT}', ended = '1' 
                                                        WHERE ${sorm_pfx}_GATEWAY.record_id = $entry->{record_id}});
                        $sth->execute() if ($debug == '0');
                }
                ...
                if($entry->{id} && exists $axbills_array{$entry->{id}})
                {
						...
						$axbills_array{$entry->{id}}->{IP_PORT} ne $entry->{IP_PORT})
							...
							my $sth = $dbh->prepare(qq{UPDATE ${sorm_pfx}_GATEWAY SET END_TIME = '$axbills_array{$entry->{id}}->{BEGIN_TIME}', ended = '1' 
											WHERE ${sorm_pfx}_GATEWAY.record_id = $entry->{record_id}});
			 				$sth->execute() if ($debug == '0');

                         				$sth = $dbh->prepare(qq{INSERT INTO ${sorm_pfx}_GATEWAY (GATE_ID, DISABLE, BEGIN_TIME, END_TIME, DESCRIPTION, IPV4, IP_PORT, ZIP, CITY, STREET, BUILDING, APARTMENT)
...
#			 $need_update_files = 1;
#                         my $sth = $dbh->prepare(qq{INSERT INTO ${sorm_pfx}_GATEWAY (GATE_ID, DISABLE, BEGIN_TIME, DESCRIPTION, IPV4, IP_PORT, ZIP, CITY, STREET, BUILDING, APARTMENT)
...
foreach my $new_entry (@new_entries) {
    ...
    # Используем // для установки значений по умолчанию
    my $sth = $dbh->prepare(qq{
        INSERT INTO ${sorm_pfx}_GATEWAY (GATE_ID, DISABLE, BEGIN_TIME, DESCRIPTION, IPV4, IP_PORT, ZIP, CITY, STREET, BUILDING, APARTMENT)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    });
...
#Export to Files
         my $sth = $dbh->prepare(qq{SELECT record_id AS id, BEGIN_TIME, END_TIME, DESCRIPTION, IPV4, IP_PORT, STREET, BUILDING, APARTMENT, ZIP, CITY 
					FROM ${sorm_pfx}_GATEWAY ORDER BY record_id ASC});
         $sth->execute or die DBI->errstr;
...
#Из базы скрипта
	 ...
	 my DBI $dbh = $self->{db}->{db};
         ...
         my $sth = $dbh->prepare(qq{SELECT record_id, ID AS id, MNEMONIC, BEGIN_TIME, END_TIME, DESCRIPTION, DATE_ADD(CURRENT_TIMESTAMP, INTERVAL $time_offset HOUR) AS CURRENT 
					FROM ${sorm_pfx}_SUPPLEMENTARY_SERVICE ORDER BY record_id DESC});
...
#AXbills
        ...
        $sth = $dbh->prepare(qq{SELECT id, DATE_ADD(last_update, INTERVAL $time_offset HOUR) AS BEGIN_TIME, name AS DESCRIPTION, module AS MNEMONIC
...
#Ищем совпадения, выпиливаем их
        foreach my $entry (@db_array)
        {
                if ($entry->{id} && !exists $axbills_array{$entry->{id}} && $entry->{END_TIME} eq $date_ended)
                {
                        $need_update_files = 1;
                        #Deleting
                        my $sth = $dbh->prepare(qq{UPDATE ${sorm_pfx}_SUPPLEMENTARY_SERVICE SET END_TIME = $entry->{CURRENT}
                                                        WHERE ${sorm_pfx}_SUPPLEMENTARY_SERVICE.record_id = $entry->{record_id}});
                        $sth->execute() if ($debug == '0');
                }
                ...
                if($entry->{id} && exists $axbills_array{$entry->{id}})
                {
                         if ($axbills_array{$entry->{id}}->{id} eq $entry->{id})
							...
							$axbills_array{$entry->{id}}->{DESCRIPTION} //= q{};$axbills_array{$entry->{id}}->{DESCRIPTION} =~ s/[;]/ /g;
                                                        ...
                                                        my $sth = $dbh->prepare(qq{UPDATE ${sorm_pfx}_SUPPLEMENTARY_SERVICE SET END_TIME = '$axbills_array{$entry->{id}}->{BEGIN_TIME}'
                                                                                        WHERE ${sorm_pfx}_SUPPLEMENTARY_SERVICE.record_id = $entry->{record_id}});
                                                        $sth->execute() if ($debug == '0');

                                                        $sth = $dbh->prepare(qq{INSERT INTO ${sorm_pfx}_SUPPLEMENTARY_SERVICE (ID, MNEMONIC, BEGIN_TIME, END_TIME, DESCRIPTION)
                                                                                VALUES ('$entry->{id}', '$axbills_array{$entry->{id}}->{MNEMONIC}', '$axbills_array{$entry->{id}}->{BEGIN_TIME}', '$date_ended',
...
#        print Dumper \@new_entries;# if ($debug == '1');
        ...
        foreach my $new_entry (@new_entries)
        {
                ...
                {
                         $need_update_files = 1;
                         my $sth = $dbh->prepare(qq{INSERT INTO ${sorm_pfx}_SUPPLEMENTARY_SERVICE (ID, MNEMONIC, BEGIN_TIME, DESCRIPTION, END_TIME)
                                        VALUES ('$new_entry->{id}', '$new_entry->{MNEMONIC}', '$new_entry->{BEGIN_TIME}', '$new_entry->{DESCRIPTION}',
                                                '$date_ended')
                         });
                         $sth->execute() if ($debug == '0');
                }
...
sub supplementary_export{
	 ...
	 my DBI $dbh = $self->{db}->{db};
#Export to Files
         my $sth = $dbh->prepare(qq{SELECT ID AS id, MNEMONIC, BEGIN_TIME, END_TIME, DESCRIPTION FROM ${sorm_pfx}_SUPPLEMENTARY_SERVICE ORDER BY record_id DESC});
         $sth->execute or die DBI->errstr;
         ...
         while(my $data = $sth->fetchrow_hashref())
         {
                print Supplementary "$data->{id};$data->{MNEMONIC};$data->{BEGIN_TIME};$data->{END_TIME};$data->{DESCRIPTION};$region_id\n";
         }
        close(Supplementary) || die;
