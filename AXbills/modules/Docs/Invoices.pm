    ...
    for ( my $i = 0; $i < ((!$user->{UID}) ? 4 : 3) + $invoice_list_fields; $i++ ){
      ...
      if ( $field_name eq 'date' ){
        ...
        $val = $html->button( $invoice->{login},
          "index=11&UID=$invoice->{uid}" ) . (($invoice->{company_name}) ? $html->br() . $html->b(
          substr( $invoice->{company_name}, 0, 30 )) : '');
      }
...
sub docs_invoice_print {
  ...
  if ( $Docs->{TOTAL} > 0 ){
    ...
    if ( $attr->{GET_EMAIL_INFO} && $attr->{SEND_EMAIL} ){
      ...
      $attr->{EMAIL_MSG_SUBJECT} = "AXbills - $lang{INVOICE}: $Doc{INVOICE_NUM}" if (!$attr->{EMAIL_MSG_SUBJECT});
    }
    ...
    if ( $Doc{IS_COMPANY} || $Doc{COMPANY_NAME} ){
      my $Customer = Customers->new( $db, $admin, \%conf );
      my $Company  = $Customer->company_from_uid( $Docs->{UID} || $FORM{UID} || $Doc{UID} );
