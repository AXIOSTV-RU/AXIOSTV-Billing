...
sub docs_tax_invoice_list{
  ...
  foreach my $line ( @{$list} ){
    my $delete = ($permissions{1}{2}) ? $html->button( $lang{DEL},
        "index=$index&del=$line->[8]",
        { MESSAGE => "$lang{DEL} ID '$line->[8]' ?", class => 'del' } ) : '';

    $table->addrow(
      $line->[0],
      $line->[1],
      $html->button( $line->[2], "index=15&UID=$line->[6]" ),
      ,
      $line->[3],
      $html->button( $line->[4], "index=11&UID=$line->[6]" ),
      $line->[5],
      $html->button( $lang{PRINT},
        "qindex=$index&print=$line->[8]&UID=$line->[6]$pages_qs" . (($conf{DOCS_PDF_PRINT}) ? '&pdf=1' : ''),
...
sub docs_tax_invoice{
  ...
  my $Company  = $Customer->company_from_uid($FORM{UID} || $LIST_PARAMS{UID});
