...
use AXbills::Base qw(days_in_month mk_unique_value encode_base64);
...
sub docs_acts_list{
  ...
  # user portal: acts of the current user
  if ($FORM{sid}){
    $index = $FORM{index} || 0;
    $FORM{subf} = $docs_acts;
  }
  ...
  foreach my $line (@{$list}) {
    ...
    $table->addrow(
      $line->{act_id},
      $line->{date},
      (defined($FORM{sid})) ? $line->{company_name} : $html->button( $line->{company_name} || $line->{fio}, "index=15&UID=$line->{uid}" ),
      $line->{sum},
...
sub docs_acts{
  ...
  my $Company  = $Customer->company_from_uid($uid);
