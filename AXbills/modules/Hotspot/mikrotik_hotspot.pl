...
=head2 USAGE
  ...
  BILLING_IP_ADDRESS - AXbills Server IP Addres
  ...
  REMOTE_ADMIN_NAME  - AXbills admin for Mikrotik Login
...
my $arguments_describe = {
  ...
  BILLING_IP_ADDRESS => {
    describe => "AXbills Server IP Address"
  },
  RADIUS_SECRET      => {
  ...
  REMOTE_ADMIN_NAME  => {
    describe => 'AXbills admin for Mikrotik Login',
...
=head2 upload_key($admin_name, $certs_dir)
  ...
  Arguments:
    $admin_name - name for AXbills administrator on Mikrotik
...
sub configure_hotspot{
  ...
  ssh(
    [
      ...
      # Prevent blocking AXbills Server
  ...
  request_value( 'UPLOAD_CAPTIVE_PORTAL', "Do you want to upload AXbills Captive portal for Hotspot", "Y" );
