...
unshift(@INC, $Bin . "/../AXbills/", $Bin . '/../', $Bin . "/../AXbills/$conf{dbtype}");


require AXbills::Base; AXbills::Base->import();
require AXbills::SQL;  AXbills::SQL->import();
require AXbills::HTML; AXbills::HTML->import();
...
my $sql       = AXbills::SQL->connect($conf{dbtype}, $conf{dbhost}, $conf{dbname}, $conf{dbuser},
...
require "AXbills/modules/Ashield/webinterface";
...
if ($conf{PAYSYS_IPS}) {
  ...
  if (! $allow) {
    ...
    sendmail("$conf{ADMIN_MAIL}", "$conf{ADMIN_MAIL}", "AXbills - Ashield", 
