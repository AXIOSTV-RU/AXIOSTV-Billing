...
sub click {
  ...
  foreach my $line (@$list) {
    ...
    if ($line->{internet_status} == 3) {
      ...
      $message .= "$self->{bot}->{lang}->{SPEED}: <b>$line->{speed} Kbit/s</b>\n" if ($line->{speed});
      $message .= "$self->{bot}->{lang}->{PRICE_MONTH}: <b>$line->{month_fee} $money_currency</b>\n" if ($line->{month_fee} && $line->{month_fee} > 0);
  ...
  $message .= "$self->{bot}->{lang}->{SUM_MONTH}: <b>$total_sum_month $money_currency</b>\n" if ($total_sum_month && $total_sum_month >0);
  $message .= "$self->{bot}->{lang}->{SUM_DAY}: <b>$total_sum_day $money_currency</b>\n";
  $message .= " <b>$money_currency</b>\n";
