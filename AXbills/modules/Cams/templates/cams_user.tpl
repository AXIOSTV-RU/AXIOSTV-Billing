...
<script>
  jQuery(document).ready(function() {
      var activateField = jQuery('#ACTIVATE');

      function fillActivateToday() {
          activateField.val(new Date().toISOString().split('T')[0]);
      }

      function shouldFillActivate() {
          var value = activateField.val();
          return !value || value == '0000-00-00';
      }

      if (jQuery('#STATUS').val() == 0 && shouldFillActivate()) {
          fillActivateToday();
      }

      jQuery('#STATUS').on('change', function() {
          if (jQuery(this).val() == 0 && shouldFillActivate()) {
              fillActivateToday();
          }
      });
  });
