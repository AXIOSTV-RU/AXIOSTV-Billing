...
sub equipment_pon_init {
  ...
  elsif ($vendor_name =~ /Gateray/i) {
    require Equipment::Gateray;
    $nas_type = '_gateray';
  }
...
sub pon_onu_state {
  ...
  my $fix_onu_config_function = $nas_type . '_fix_onu_config';
...
# print Dumper($select_input_tooltip);
  my @info = ([
    ...
    $html->button("$pon_type " . ($attr->{BRANCH} || q{}) . ((defined $attr->{ONU_ID}) ? ":$attr->{ONU_ID}" : q{}), 'test',
      ...
      . (($get_onu_config_function && defined(&{$get_onu_config_function})) ?
        ...
        : "")
      . (($fix_onu_config_function && defined(&{$fix_onu_config_function}) && $onu_abon->[0]->{vlan} && $onu_abon->[0]->{vlan} > 0) ?
        $html->button($lang{FIX_ONU_CONFIG},
          "NAS_ID=$nas_id&index=" . get_function_index('equipment_info')
        . "&visual=4&ONU=$id&info_pon_onu=" . ($attr->{ONU_SNMP_ID} || q{}) . "&ONU_TYPE=$pon_type&fix_onu_config=1",
          { class => 'btn btn-sm btn-success', TITLE => $lang{FIX_ONU_CONFIG} } )
        : "")
      . "($id)"
  ]);
  ...
  if ($FORM{fix_onu_config}) {
    if ($fix_onu_config_function && defined(&{$fix_onu_config_function})) {
      my @onu_config_arr = &{\&$fix_onu_config_function}({%$attr, PON_TYPE => $pon_type, INTERNET_PLUS_VLAN => $onu_abon->[0]->{vlan}});
    }
  }
