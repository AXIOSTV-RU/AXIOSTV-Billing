...
sub user_portal {
  ...
  my $url = qq[$url_conf/payment/rest/register.do?amount=$amount&language=ru&orderNumber=$attr->{OPERATION_ID}&]
    . qq[password=] . urlencode($self->{conf}{PAYSYS_YKASSA_PASSWORD}) . qq[&userName=$self->{conf}{PAYSYS_YKASSA_USERNAME}&]
    . qq[jsonParams=];

  $url .= urlencode(qq[{"uid":$user->{UID}}]);
  $url .= "&returnUrl=" . urlencode("$ENV{PROT}://$ENV{SERVER_NAME}:$ENV{SERVER_PORT}/index.cgi?index=$attr->{index}&PAYMENT_SYSTEM=$attr->{PAYMENT_SYSTEM}&TRANSACTION_ID=YK:$attr->{OPERATION_ID}");
