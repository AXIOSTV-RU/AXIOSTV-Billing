...
package Paysys::systems::VTB;
our $PAYSYSTEM_NAME = 'VTB';
our $PAYSYSTEM_SHORT_NAME = 'VTB';
our $PAYSYSTEM_ID = 123;

our $PAYSYSTEM_VERSION = '7.00';

our %PAYSYSTEM_CONF = (
  PAYSYS_VTB_ACCOUNT_KEY => '',
);
...
sub proccess {
  ...
  my $CHECK_FIELD       = $self->{conf}{PAYSYS_VTB_ACCOUNT_KEY} || 'UID';
  ...
  # QueryType=check&account=1&summa=1.00&payment_id=12345678
    ...
    my ($pay_result, $pay_id) = main::paysys_pay({
        ...
        MK_LOG            => 1,
        PAYMENT_ID        => 1,
        CURRENCY_ISO      => $self->{conf}{"PAYSYS_" . $PAYSYSTEM_NAME . "_CURRENCY"},
        PAYMENT_DESCRIBE  => $FORM->{payment_describe} || "$PAYSYSTEM_NAME Payments",
        DEBUG             => $self->{DEBUG},
    });
