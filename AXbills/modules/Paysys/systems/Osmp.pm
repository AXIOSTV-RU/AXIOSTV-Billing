...
sub check {
  ...
  if (!$self->{conf}{PAYSYS_PEGAS} && !$self->{conf}{"PAYSYS_" . $CUSTOM_NAME . "_EXT_PARAMS"}) {
    if($CUSTOM_ID == 151){
      $RESULT_HASH{'fields'} .= qq{<field1 name=\"fio\">$user_object->{fio}</field1><field2 name=\"balance\">$user_object->{deposit}</field2>};
      $RESULT_HASH{$txn_id} = $FORM->{txn_id};
      $RESULT_HASH{$trm_id} = $FORM->{'trm_id'} if ($FORM->{'trm_id'});
    } else {
      $RESULT_HASH{$txn_id} = $FORM->{txn_id};
      $RESULT_HASH{$trm_id} = $FORM->{'trm_id'} if ($FORM->{'trm_id'});
