...
=head2 paysys_pay() - make payment;
  ...
  Arguments:
    $attr
      ...
      ORDER_ID                - Transaction identifier in AXbills;
      MK_LOG                  - Logging;
      ...
      PAYMENT_ID        - if this attribute is on(1), function will return two values:
                                    ...
                                    $payments_id - transaction identifier in AXbills;
...
sub paysys_pay {
  ...
  my $user_pi = $users->pi({ UID => $uid, COLS_NAME => 1});
  $user->{FIO} = $user_pi->{FIO};
  ...
  $payments->add($user, {
    ...
    METHOD         => $method || (($conf{PAYSYS_PAYMENTS_METHODS} && $PAYSYS_PAYMENTS_METHODS{$payment_system_id}) ? $payment_system_id : '2'),
    EXT_ID         => "$payment_system:$ext_id$trm_id",
  ...
  if ($conf{PAYSYS_TG_NOTICE}) {
    ...
    my $sent_message = $Sender->send_message({
        AID         => $conf{AID_FOR_NOTICE},
        SUBJECT     => "Пополнение счёта через $attr->{PAYMENT_DESCRIBE}",
        MESSAGE     => $message,
...
sub paysys_import_parse {
  ...
  print "DEBUG Enabled!!!, EXPRESSION: $expration\nColumns: $columns\n" if ($debug > 0);
  ...
  for (my $row = $first_row; $row <= $#rows; $row++) {
    ...
    if (my @res = ($line =~ /$expration/)) {
      ...
      print $html->b("$lang{ERROR}: line: $line_count") . " '$line'" . $html->br() if ($debug > 0);
    }
