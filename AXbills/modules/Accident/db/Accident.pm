...
sub list {
  ...
  my $WHERE = $self->search_former(
    ...
    [
      ...
      [ 'AID',                     'INT',    'al.aid',                  1],
      [ 'ADMIN_NAME',              'STR',    'a.name', 'a.name AS admin_name',  1],
      ...
      [ 'FROM_DATE|TO_DATE',       'DATE',   "DATE_FORMAT(al.date, '%Y-%m-%d %H:%i')", ],
      [ 'SENT_OPEN',		   'INT',    'al.sent_open',		],
      [ 'SENT_CLOSE',		   'INT',    'al.sent_close',		],
    ],
    {
      WHERE       => 1,
  ...
  SELECT $self->{SEARCH_FIELDS} 
    al.id, al.date, al.name, 
    al.descr, al.priority, al.aid, a.name AS admin_name, 
    al.status, al.end_time, al.realy_time, 
    GROUP_CONCAT(
      CASE
        WHEN ad.type_id= 1 
            THEN di.name
...
sub accident_report {
  ...
  if ($attr->{ALL}) {
    $date = "DATE_FORMAT(al.date, '%Y-%m-%d') AS date";
    push @WHERE_RULES, "al.id > '0'";
  }
  if ($attr->{FROM_DATE_TO_DATE}) {
    ($attr->{FROM_DATE}, $attr->{TO_DATE}) = split(/\//, $attr->{FROM_DATE_TO_DATE}, 2);
  }

  my $WHERE =  $self->search_former($attr, [
      ['PRIORITY',		'INT',	'al.priority',                      ],
      ['AID',			'INT',	'al.aid',                           ],
      ['MONTH',			'DATE',	"DATE_FORMAT(al.date, '%Y-%m')"     ],
  ...
  my $type = $attr->{TYPE} || q{};
    ...
    if ($type eq 'PER_MONTH') {
      $date = "DATE_FORMAT(al.date, '%Y-%m') AS date, DATE_FORMAT(al.date, '%Y-%m') AS month";
    }
    elsif ($type eq 'DAYS') {
      $date = "DATE_FORMAT(al.date, '%Y-%m-%d') AS date";
    }
  ...
  $self->query("SELECT $date,
    ...
    SUM(TIMESTAMPDIFF(HOUR, al.date, al.realy_time)) AS hour_diff
    FROM accident_log al
    $WHERE
    GROUP BY 1
    ORDER BY $SORT $DESC;",
...
sub user_accident_list {
  ...
  $self->query(
    "SELECT id, name, descr, priority, aid, status, address_id, type_id, date, end_time, realy_time, uid FROM (
      SELECT al.id as id1, al.name, al.descr, al.priority, al.aid, al.status, ad.address_id, ad.type_id, al.id, al.date, al.end_time, al.realy_time, u.uid
        FROM accident_log al
          LEFT JOIN admins a ON al.aid = a.aid
          LEFT JOIN accident_address ad ON (ad.ac_id = al.id)
      ...
      WHERE u.uid = ? AND al.status=0
        UNION ALL
      SELECT al.id as id3, al.name, al.descr, al.priority, al.aid, al.status, ad.address_id, ad.type_id,  al.id, al.date, al.end_time, al.realy_time, u.uid
        FROM accident_log al
          LEFT JOIN admins a ON (al.aid = a.aid)
          LEFT JOIN accident_address ad ON (ad.ac_id = al.id)
        ...
        WHERE u.uid = ? AND al.status=0
          UNION ALL
      SELECT al.id as id4, al.name, al.descr, al.priority, al.aid, al.status, ad.address_id, ad.type_id,  al.id, al.date, al.end_time, al.realy_time, u.uid
        FROM accident_log al
          LEFT JOIN admins a ON (al.aid = a.aid)
          LEFT JOIN accident_address ad ON (ad.ac_id = al.id)
    ...
    GROUP BY user_accident_cabinet.id
    ORDER BY date", undef, {
      %{ $attr },
      Bind => [ 
        $uid,
...
b.coordx,
b.coordy   
FROM accident_address ad
LEFT JOIN accident_log al ON al.id = ad.ac_id
LEFT JOIN builds b ON b.id = ad.address_id
LEFT JOIN streets st_b ON st_b.id = b.street_id
LEFT JOIN districts di_b ON (di_b.id = st_b.district_id )
LEFT JOIN streets st ON st.id = ad.address_id
LEFT JOIN districts di_s ON (di_s.id = st.district_id)
LEFT JOIN districts di ON (di.id = ad.address_id);",
    undef,
    { COLS_NAME => 1 }
  );
...
sub accident_equipment_list {
  ...
  my $WHERE = $self->search_former(
    ...
    [
      ...
      [ 'AID',            'INT',    'ae.aid',                         1 ],
      [ 'ADMIN_NAME',	  'STR',    'ad.name AS admin_name',	      1 ],
  ...
  $self->query("SELECT $self->{SEARCH_FIELDS} ae.id 
	FROM accident_equipments AS ae 
	LEFT JOIN admins ad ON ae.aid = ad.id
	$ext_table 
	$WHERE 
	GROUP BY ae.id
	ORDER BY $SORT $DESC;",
...
#**********************************************************
=head2 notification_add ($attr) - add accident sent notification 
...
sub notification_add {
  ...
  $self->query_add('accident_notifications', $attr);

  return $self;
}

#**********************************************************
=head2 notification_info ($attr) - info accident sent notification 
...
=cut
#**********************************************************
sub notification_info {
  my $self = shift;
  my ($attr) = @_;

  $self->query("SELECT 1 FROM accident_notifications WHERE accident_id = ? AND uid = ? AND notify_type = ?;", undef, { Bind => [ $attr->{ACCIDENT_ID}, $attr->{UID}, $attr->{NOTIFY_TYPE} ] } );

  return $self->{list};
}

#**********************************************************
=head2 notification_del ($attr) - del accident sent notification
...
=cut
#**********************************************************
sub notifications_del {
  my $self = shift;
  my ($attr) = @_;

  $self->query("DELETE FROM accident_notifications WHERE accident_id = ?", 'do', {
    Bind => [ $attr->{ID} ]
  });
