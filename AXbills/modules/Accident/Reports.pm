...
#print Dumper $accident_report;
  my $table = $html->table({
    width   => '100%',
    caption => $lang{REPORTS},
    border  => 1,
    title   => [ (defined($FORM{TYPE}) eq 'PER_MONTH') ? ($lang{MONTH}) : ($lang{DATE}), $lang{ACCIDENT_QUANTITY}, $lang{HOURS} ],
    ID      => 'ACCIDENT_REPORT',
    EXPORT  => 1,
  });
  ...
  foreach my $line (@$accident_report) {
    ...
    if (defined($FORM{TYPE}) && $FORM{TYPE} eq 'PER_MONTH') {
