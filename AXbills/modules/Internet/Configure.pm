  ...
  if ($FORM{ADD_TP}) {
    ...
    $Tariffs->{NAME_SEL} = $html->form_main({
      CONTENT => $html->form_select('TP_ID', {
        ...
        SEL_LIST  => $Tariffs->list({ %LIST_PARAMS, MODULE => 'Internet', NEW_MODEL_TP => 1, SORT => 1, COLS_NAME => 1 }),
        SEL_KEY   => 'tp_id',
        SEL_VALUE => 'id,name',
        AUTOSUBMIT => 'form',
        NO_ID     => 1
...
sub internet_tp_form {
  ...
  $html->tpl_show(_include('internet_tp', 'Internet'), $tarif_info, { SKIP_VARS => 'IP', ID => 'internet_tp' });
	} else { $html->tpl_show(_include('internet_tp_mod', 'Internet'), $tarif_info, { SKIP_VARS => 'IP', ID => 'internet_tp' }); }
	## END AXbills
