...
sub internet_user {
  ...
  if (!$Internet->{TOTAL} || $Internet->{TOTAL} < 1) {
    ...
    $Internet->{PORT_FILL} = ($Internet->{PORT_FILL}) ? 'checked' : '';
  ...
  if ($Internet->{CPE_MAC}) {
	...
	### START AXbills
	if($conf{INTERNET_FORM_ACTIVATE_DATE}){
		$Internet->{FLAG_DATE_TODAY} = 1;
  }
	### END AXbills
...
sub internet_join_service {
  ...
  my $uid = $Internet->{UID};
  return '' unless $users->{IS_COMPANY};
...
sub internet_registration_info {
  ...
  if ($user->{IS_COMPANY}) {
    $company_info = {
      NAME             => $pi->{COMPANY_NAME} || $user->{COMPANY_NAME} || '',
      TAX_NUMBER       => $pi->{INN} || '',
      KPP              => $pi->{KPP} || '',
      OGRN             => $pi->{OGRN} || '',
      OKPO             => $pi->{OKPO} || '',
      BANK_BIC         => $pi->{BANK_BIC} || '',
      BANK_NAME        => $pi->{BANK_NAME} || '',
      BANK_ACCOUNT     => $pi->{BANK_ACCOUNT} || '',
      COR_BANK_ACCOUNT => $pi->{COR_BANK_ACCOUNT} || '',
    };
  }
