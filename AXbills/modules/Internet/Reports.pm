...
sub users_development_report {
  ...
  my $table = $html->table({
    ...
    qs              => $pages_qs,
	###
    class           => 'table table-hover table-condensed table-striped table-bordered table-head-fixed text-center align-middle',
	###
  ...
  $table->addtd($table->td($lang{TOTAL}, { colspan => 2, class => 'skip text-right font-italic align-middle' }), @{$total_growth},
    @{$start_total}, @{$end_total});

  foreach my $city (sort keys %{$cities_start_period}) {
    $table->addtd($table->td($city, { colspan => 2, class => 'skip text-right font-italic align-middle' }), @{$growth_by_city{$city}},
  ...
  foreach my $city (sort keys %rows_by_city) {
    ...
    foreach my $district (@district_keys) {
      ...
      push(@row, [ $table->td($district, { class => 'skip text-right font-italic align-middle' }), @{$growth_by_district{$city}{$district}},
...
sub _value_format {
  ...
  $value .= '%' if $format->{percent};
	...
	###
  return $table->td($value, { class => 'text-center align-middle' });
	###
}
