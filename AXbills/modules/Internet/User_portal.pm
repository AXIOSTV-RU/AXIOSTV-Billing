...
## START AXbills ### 
  my $show_only_holdup = $LIST_PARAMS{SHOW_ONLY_HOLDUP} || 0;
## END AXbills ### 
  if (!$FORM{ID}) {
...
## START AXbills ### 
    if ($Internet->{TOTAL_SERVICES} > 1) {
      foreach my $line (@$list) {
        ...
        internet_user_info_proceed({ ID => $line->{id}, UID => $uid, SHOW_ONLY_HOLDUP => $show_only_holdup }); 
      }
      return 1;
    }
  }
  ...
  internet_user_info_proceed({ UID => $uid,  ID => $FORM{ID}, SHOW_ONLY_HOLDUP => $show_only_holdup });
  ## END AXbills ### 
  return 1;
}
...
sub internet_user_info_proceed {
  ...
  ## START AXbills ### 
  my $show_only_holdup = $attr->{SHOW_ONLY_HOLDUP};
  ## END AXbills ### 
  ...
  ## START AXbills ### 
  $Internet->{SHOW_ONLY_HOLDUP} = $show_only_holdup;
  ## END AXbills ### 
...
## END AXbills ###
  ...
  my @check_fields = (
    ...
    "CID::MAC_LK:"
      . (($conf{INTERNET_MAC_DICOVERY}) ? $html->button($lang{CHANGE_MAC_LK}, "index=". get_function_index('internet_user_info')
...
## START AXbills ### 
    if($Internet->{SHOW_ONLY_HOLDUP}){
      return $html->tpl_show(_include('internet_hold_up_only', 'Internet'), $Internet);
    }
