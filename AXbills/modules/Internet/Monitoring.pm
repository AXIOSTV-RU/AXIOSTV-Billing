...
sub internet_online {
    ...
    my $sth_online = $dbh->prepare("SELECT uid FROM internet_online WHERE guest = 1");
    $sth_online->execute;

    my @uids;
    while (my $row = $sth_online->fetchrow_arrayref) {
        push @uids, $row->[0];
    }

    my $count_debetor = 0;
    if (@uids) {
        my $placeholders = join(',', ('?') x @uids);
        my $sth_main = $dbh->prepare("SELECT COUNT(*) FROM internet_main WHERE uid IN ($placeholders) AND disable = 5");
        $sth_main->execute(@uids);
        ($count_debetor) = $sth_main->fetchrow_array;
    }

    my $count_guest = 0;
    if (@uids) {
        my $placeholders = join(',', ('?') x @uids);
        my $sth_main_not_disabled = $dbh->prepare("SELECT COUNT(*) FROM internet_main WHERE uid IN ($placeholders) AND disable != 5");
        $sth_main_not_disabled->execute(@uids);
        ($count_guest) = $sth_main_not_disabled->fetchrow_array;
    }

    $guest = $count_guest;
    $debetor = $count_debetor;
    $sth_online->finish;
    $dbh->disconnect();
  ...
  my %EXT_TITLES = (
    'fio'                 => $lang{FIO},
    'online_company_name' => $lang{COMPANY},
  ...
  ($table, $list) = result_former({
    INPUT_DATA      => $Sessions,
    FUNCTION        => 'online',
    DEFAULT_FIELDS  => 'LOGIN,DURATION_SEC2,CLIENT_IP_NUM,ACCT_INPUT_OCTETS,ACCT_OUTPUT_OCTETS,ONLINE_COMPANY_NAME,FIO',
