    ...
    if ($sprint_start_sec <= $period_end_sec && $sprint_start_sec > $period_start_sec) {
      ...
      my $table_sprint = $html->table({
        ...
        title   => [
          ...
          "$lang{SUCCESS2} ($lang{AMOUNT_POINTS},$lang{PLAN} / $lang{SCHEDULED_HOURS})",
          "$lang{SUCCESS2} ($lang{AMOUNT_POINTS},$lang{FACT} / $lang{ACTUALLY_HOURS})"
        ],
        qs      => $pages_qs,
        ID      => "SPRINT_REPORT",
        EXPORT  => 1
...
sub _sprint_report_chart {
  ...
  print $html->chart({
    ...
    DATA              => {
      ...
      "$lang{SUCCESS2}, $lang{PLAN}"                => $success_plan,
      "$lang{SUCCESS2}, $lang{FACT}"                => $success_fact
    },
    BACKGROUND_COLORS => {
      ...
      "$lang{SUCCESS2}, $lang{PLAN}"                => 'rgba(220, 53, 69, 0.5)',
      "$lang{SUCCESS2}, $lang{FACT}"                => 'rgba(220, 53, 69, 0.8)'
    },
    OUTPUT2RETURN     => 1,
    FILL              => 'false',
    IN_CONTAINER      => 1
