...
sub is_user_online {
    my ($uid) = @_;
    my $query = "SELECT 1 FROM internet_online WHERE uid = ?";
    my $sth = $dbh->prepare($query);
    $sth->execute($uid);
    my $is_online = $sth->fetchrow_array ? 1 : 0;
    $sth->finish;
    return $is_online;
}
...
sub _msgs_dispatches_job_list {
  ...
  foreach my $message (@{$messages}) {
    ...
    if ($message->{uid}) {
     ...
     my $is_online = is_user_online($message->{uid});
     my $online_status = $is_online ? "Online" : "Offline"; 
      
      my $color = $is_online ? "limegreen" : "red"; 
      
      $user_link = $html->button(
        $user_list->[0]{LOGIN} . " (<span style='color: $color;'>$online_status</span>)",
        "index=" . get_function_index("form_users") . "&UID=$message->{uid}",
        {
...
sub msgs_dispatch {
  ...
  if ($FORM{add} || $FORM{add_modal}) {
    ...
    foreach my $line (@$dispatch_admins) {
      $brigade .= "$line->{name} _____________";
      $brigade .= $html->br();
