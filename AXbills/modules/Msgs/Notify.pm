...
sub notify_admins {
  ...
  $Sender->send_message_auto({
    ...
    ACTIONS     => $preview_url,
    MAIL_HEADER => [ "X-AXbills-Msg-ID: $message_id", "X-AXbills-REPLY-ID: $reply_id", "Content-Type: text/html;" ],
...
sub notify_user {
  ...
  foreach my $user_info  ( @{$users_list} ) {
    ...
    $Sender->send_message_auto({
      ...
      ATTACHMENTS => ($#{$ATTACHMENTS} > -1) ? $ATTACHMENTS : undef,
      ACTIONS     => $preview_url,
      MAIL_HEADER => [ "X-AXbills-Msg-ID: $message_id", "X-AXbills-REPLY-ID: $reply_id" ],
      MAKE_REPLY  => $message_id,
      LANG        => \%lang,
      PARSE_MODE  => 'HTML',
      ALL         => 1,
