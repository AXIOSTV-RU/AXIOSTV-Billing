...
sub customers_list{
  ...
  if ( defined( $attr->{USER_TYPE} ) && $attr->{USER_TYPE} ne '' ){
    push @WHERE_RULES, ($attr->{USER_TYPE} == 1) ? "u.is_company=1" : "u.is_company=0";
  }
  ...
  my $ADDRESS_FULL = ($CONF->{ADDRESS_REGISTER}) ? "concat(streets.name, '$CONF->{BUILD_DELIMITER}', builds.number, '$CONF->{BUILD_DELIMITER}', pi.address_flat) AS ADDRESS" : "concat(pi.address_street, '$CONF->{BUILD_DELIMITER}', pi.address_build, '$CONF->{BUILD_DELIMITER}', pi.address_flat) AS ADDRESS";

  $self->query( "SELECT
                         u.uid, 
                         if(u.is_company=1 AND pi.company_name<>'', pi.company_name,
                         ...
                         $ADDRESS_FULL,
                         pi.phone,
                         pi.contract_sufix AS contract_sufix,
                         pi.contract_id AS contract_id,
                         pi.contract_date AS contract_date,
                         u.bill_id AS bill_id,
                         pi.bank_account AS bank_account,
                         pi.bank_name AS bank_name,
                         pi.cor_bank_account AS cor_bank_account,
                         u.uid
                       $self->{SEARCH_FIELDS}
...
sub payment_deed{
  ...
  if ( defined( $attr->{USER_TYPE} ) && $attr->{USER_TYPE} ne '' ){
    push @WHERE_RULES, ($attr->{USER_TYPE} == 1) ? "u.is_company=1" : "u.is_company=0";
    push @WHERE_RULES_DV, ($attr->{USER_TYPE} == 1) ? "u.is_company=1" : "u.is_company=0";
  }
  ...
  u.bill_id AS bill_id,
  SUM(f.sum) AS sum,
  IF(u.is_company=1 AND pi.company_name<>'', pi.company_name, if(pi.fio<>'', pi.fio, u.id)) AS fio,
  u.is_company,
  0 AS vat,
  u.uid,
...
sub extfin_report_balances{
  ...
  $self->query( "SELECT report.id,
   u.id,
   IF(u.is_company=1 AND pi.company_name<>'', pi.company_name, IF(pi.fio<>'', pi.fio, u.id)) AS user_name,
...
sub extfin_debetors{
  ...
  if ( $attr->{DATE} ){
    ...
    push @WHERE_RULES, "b.deposit < 0";    # and (f.last_deposit >=0 and f.last_deposit-sum<0)";
    $ext_field = "\@A:=b.deposit AS debet,";
    $attr->{DATE} = 'CURDATE()';
  }
...
sub report_users_balance{
  ...
  if ( $self->{TOTAL} > 0 || $PG > 0 ){
    $self->query( "SELECT count(DISTINCT u.uid) AS users_total,
       SUM(b.deposit) AS payments_total,
       SUM(u.credit) AS fees_sum
     FROM users u
     LEFT JOIN bills b ON (u.bill_id = b.id)
    WHERE u.deleted=0 $WHERE;",
      undef,
