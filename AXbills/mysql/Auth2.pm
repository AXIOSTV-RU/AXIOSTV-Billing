...
#  }
  ...
  my $pppoe_vlan_auth = '';
  my $ignore_cid = 0;
  my $ignore_pppoeplus = 0;

  if ($CONF->{INTERNET_CID_SKIP}) {
     $ignore_cid  = 1;
  }

  if ($CONF->{INTERNET_PPPOEPLUS_SKIP_AUTH}) {
     $ignore_pppoeplus  = 1;
  }

  if ($CONF->{INTERNET_PPPOE_AUTH_VLAN}) {
      $pppoe_vlan_auth = $RAD->{'Called-Station-Id'};

      #Обрезаем название интерфейса. Оставляем только VLAN
      $pppoe_vlan_auth =~ s/^(.*)\.(.*)$/$2/g;

      if (! $self->{VLAN}) {
        $self->query2("UPDATE internet_main SET vlan='$pppoe_vlan_auth' WHERE uid='$self->{UID}';", 'do');
        $self->{VLAN}=$pppoe_vlan_auth;
      }
  }
...
#      if ($self->{VLAN} && $self->{VLAN} !~ /any/i) {
#        $ignore_cid  = 1;
#      }
#      elsif (! $self->{VLAN}) {
      if (!$self->{PORT_OPT82} || ($CONF->{INTERNET_PPPOEPLUS_FORCE_UPDATE} && $ignore_pppoeplus)) {
        $self->query2("UPDATE internet_main SET port_opt82='$pppoe_pluse' WHERE uid='$self->{UID}';", 'do');
        $self->{PORT_OPT82}=$pppoe_pluse;
      }
    }
    else {
...
#   if ($self->{PORT} && $self->{PORT} !~ m/any/i && $self->{PORT} ne $pppoe_pluse) {
   if (!$ignore_pppoeplus && $pppoe_pluse && $self->{PORT_OPT82} && $self->{PORT_OPT82} !~ m/any/i && $self->{PORT_OPT82} ne $pppoe_pluse) {

    $RAD_PAIRS->{'Reply-Message'} = "Incorrect Switch port STORED: '$self->{PORT_OPT82}' REQ: '$pppoe_pluse'";
    return 1, $RAD_PAIRS;
  }
...
#Обрезаем название интерфейса. Оставляем только VLAN
   ...
   if ($CONF->{INTERNET_PPPOE_AUTH_VLAN} && $pppoe_vlan_auth && $self->{VLAN} !~ m/any/i && $self->{VLAN} ne $pppoe_vlan_auth) {
    $RAD_PAIRS->{'Reply-Message'} = "WRONG_VLAN_ID STORED: '$self->{VLAN}' REQ: '$pppoe_vlan_auth'";
    return 1, $RAD_PAIRS;
  }
...
#    return 1, $RAD_PAIRS;
  ...
  elsif ($self->{ACCOUNT_AGE} > 0) {
    my $to_expire = $self->{DAY_BEGIN} + $self->{ACCOUNT_AGE} * 86400 - $self->{SESSION_START};
    if ($to_expire < $time_limit) {
      $time_limit = $to_expire;
    }
  }
  ...
  if ( $CONF->{MAC_AUTO_ASSIGN} && $CONF->{MAC_AUTO_ASSIGN_FORCE} && $CONF->{INTERNET_CID_SKIP} 
    && ($self->{CID} ne $cid)
    && ($cid =~ /:|\-/ ))
 {
        if ($cid =~ /\/\s+([A-Za-z0-9:]+)\s+\//) {
...
sub get_ip {
  ...
  while ($#ips_arr > -1) {
    my $assign_ip = splice(@ips_arr, int(rand($#ips_arr + 1)), 1);
...
sub get_ip2 {
  ...
  if ($attr->{TP_IPPOOL}) {
    $WHERE .= " AND ippools.id='$attr->{TP_IPPOOL}'";
    delete ($attr->{TP_IPPOOL});
  }
  else {
    $WHERE .= " AND nas_ippools.nas_id = '$nas_num'";
    if($guest_mode) {
      #Only guest pool
      if($attr->{SERVER_VLAN}) {
        $WHERE .= " AND ippools.vlan='$attr->{SERVER_VLAN}'";
...
sub dhcp_info {
  ...
  if ($CONF->{AUTH_IP} && $attr->{IP}) {
    ...
    if ($CONF->{AUTH_PARAMS} ne '1') {
      ...
      my $nas_mac  = $attr->{NAS_MAC}  || '';
      my $user_mac = $attr->{USER_MAC} || '';
      my $vlan     = $attr->{VLAN}     || 0;
      my $port     = $attr->{PORT}     || 0;

      my $mac_match = '0';
      if ($user_mac ne '' && uc($user_mac) ne 'ANY') {
        $mac_match = "((n.mac='$nas_mac' OR n.mac IS null)
          AND internet.cid='$user_mac'
          AND (internet.vlan='$vlan' OR internet.vlan='')
          AND (internet.port='$port' OR internet.port=''))";
      }

      my $any_match = '0';
      if ($nas_mac ne '' && $port && $port ne '0') {
        $any_match = "((internet.cid='' OR UPPER(internet.cid)='ANY')
          AND internet.nas_id>0
          AND internet.port<>'' AND internet.port<>'0'
          AND n.mac='$nas_mac'
          AND internet.port='$port'
          AND (internet.vlan='$vlan' OR internet.vlan=''))";
      }

      push @WHERE_RULES, "(($mac_match) OR ($any_match))";
    }
