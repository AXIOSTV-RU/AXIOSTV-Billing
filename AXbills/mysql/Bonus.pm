...
sub accomulation_rule_list {
  ...
  my @WHERE_RULES = ("tp.module IN ('Internet')");
...
### START AXbills
#**********************************************************
=head2 add_ext_bill_id($attr) -- adding ext_bill for a subscriber who does not have it
# Return the new ext_id of the account
=cut
#**********************************************************
sub add_ext_bill_id {
  my $self   = shift;
  ...
  $self->query("INSERT INTO bills (uid, registration) VALUES ($attr->{UID}, CURRENT_DATE);", 'do');

  my $new_bill_id = $self->{INSERT_ID};

  $self->query("UPDATE users SET ext_bill_id = $new_bill_id WHERE uid = $attr->{UID};",'do');
