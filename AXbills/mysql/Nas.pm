...
sub list {
  ...
  my $WHERE =  $self->search_former($attr, [
      ...
      ['ACCIDENT_CHECK',   'INT', 'nas.accident_check',		       1 ],
  ...
  if (! $attr->{SHORT}) {
    ...
    nas.accident_check as nas_accident_check,
    nas.descr,
...
sub info {
  ...
  $self->query2(
    "SELECT id as nas_id,
      ...
      accident_check AS nas_accident_check,
...
sub change {
  ...
  $attr->{NAS_ACCIDENT_CHECK} = ($attr->{NAS_ACCIDENT_CHECK}) ? 1 : 0;
  ...
  my %FIELDS = (
    ...
    NAS_ALIVE        => 'alive',
    NAS_DISABLE      => 'disable',
    NAS_ACCIDENT_CHECK => 'accident_check',
...
sub add {
  ...
  $self->query_add('nas', {
    ...
    ACCIDENT_CHECK => $attr->{NAS_ACCIDENT_CHECK},
