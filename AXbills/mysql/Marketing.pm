...
sub internet_fees_monitor {
  ...
  FROM users u
  INNER JOIN internet_main dv on (dv.uid=u.uid)
  INNER JOIN tarif_plans tp on (dv.tp_id=tp.tp_id)
...
\@user_deposit := b.deposit AS user_deposit,
\@user_credit := u.credit AS user_credit,
...
INNER JOIN internet_main  dv ON (u.uid=dv.uid)
INNER JOIN tarif_plans tp ON (tp.tp_id=dv.tp_id)
...
sub triplay_stats {
  ...
  LEFT JOIN internet_main dv ON (pi.uid=dv.uid)
   LEFT JOIN tarif_plans dv_tp ON (dv.tp_id=dv_tp.tp_id AND module='Internet')
...
INNER JOIN users u ON (u.uid=dhcp.uid)
  ...
  LEFT JOIN internet_main dv ON (dv.uid=u.uid)
