...
#      ['PAYMENT_ID',     'INT', 'd.payment_id',                      ],
  ...
  my $WHERE = $self->search_former($attr, [
   ...
   pi.contract_id, pi.contract_date,  u.bill_id AS bill_id,  pi.email,  pi.fio" ],
    ['UID',            'INT', 'd.uid',                             ],
...
sub tax_invoice_list {
  ...
  my $EXT_TABLES = "LEFT JOIN users u ON (d.uid=u.uid)
      LEFT JOIN users_pi pi ON (pi.uid=u.uid)";
  if ($attr->{FULL_INFO}) {
    $self->{EXT_FIELDS} = ",
...
sub acts_list {
  ...
  my $WHERE =  $self->search_former($attr, [
    ...
    ['COMPANY_NAME',   'STR', 'pi.company_name', 'pi.company_name AS company_name' ],
  ...
  $self->query("SELECT d.act_id, d.date, pi.company_name AS company_name,
