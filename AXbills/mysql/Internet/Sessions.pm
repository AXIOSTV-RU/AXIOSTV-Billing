...
sub online {
  ...
  $WHERE = $self->search_former($attr, [
      ...
      ['ONLINE_COMPANY_NAME','STR','pi.company_name AS online_company_name',       1 ],
...
sub list {
  ...
  if (defined($attr->{CID})) {
    $attr->{CID} =~ s/[.\:\-]//g;
    $attr->{CID} =~ s/(\b|\G)[\da-f]{2}(?=[\da-f]{2})/$&:/ig;
  }
...
sub reports {
  ...
  if (defined($attr->{DATE})) {
    ...
    if ($attr->{TYPE} eq 'HOURS') {
      ...
      $EXT_TABLES = "INNER JOIN users_pi pi ON (pi.uid=u.uid)";
      push @WHERE_RULES, "u.is_company=1";
    }
    else {
      $date = "u.id";
    }
  ...
  my $WHERE = $self->search_former($attr, [
      ...
      [ 'GID',          'INT',  'u.gid',  ],
      [ 'UID',          'INT',  'l.uid',  ],
      [ 'IS_COMPANY',   'INT',  'u.is_company' ],
      [ 'COMPANY_NAME', 'STR',  'pi.company_name' ],
      [ 'TP_ID',        'INT',  'l.tp_id' ],
      [ 'GUEST',        'INT',  'l.guest' ]
    ],
    { WHERE             => 1,
...
sub reports2 {
  ...
  my $WHERE2 =  $self->search_former($attr, [
      ...
      ['TP_ID',           'INT', 'l.tp_id',            1 ],
      ['COMPANIES',       'STR', 'pi.company_name',    'pi.company_name AS company_name' ],
  ...
  if ( $attr->{INTERVAL} ){
    ...
    if ( $attr->{TYPE} eq 'HOURS' ){
      ...
      $main_field = "pi.company_name AS company_name";
      $EXT_TABLE_JOINS_HASH{users} = 1;
      $EXT_TABLE_JOINS_HASH{users_pi} = 1;
      $WHERE2 .= ($WHERE2 ? ' AND' : 'WHERE') . " u.is_company=1";
    }
