...
sub report_debetors {
  ...
  $self->query("SELECT
    ...
    LEFT JOIN tarif_plans tp ON (tp.tp_id=internet.tp_id)
    $EXT_TABLES
    WHERE b.deposit < 0 - tp.month_fee*$attr->{PERIOD} $WHERE
    GROUP BY u.id
    ORDER BY $SORT $DESC LIMIT $PG, $PAGE_ROWS;",
    undef,
    $attr
  ...
  if ($self->{TOTAL} >= 0 && !$attr->{SKIP_TOTAL}) {
    $self->query("SELECT COUNT(*) AS total, SUM(b.deposit) AS total_debetors_sum
      FROM users u
    INNER JOIN internet_main internet ON (u.uid=internet.uid)
    LEFT JOIN tarif_plans tp ON (tp.tp_id=internet.tp_id)
    $EXT_TABLES
    WHERE b.deposit < 0 - tp.month_fee*$attr->{PERIOD}
    $WHERE",
    undef,
    { INFO => 1 }
    );
...
sub report_tp {
  ...
  $self->query("SELECT tp.id, tp.name, COUNT(DISTINCT internet.uid) AS counts,
      ...
      COUNT(DISTINCT CASE WHEN b.deposit < 0 THEN internet.uid ELSE NULL END) AS debetors,
...
sub report_user_statuses {
  ...
  $self->query("SELECT COUNT(i.uid) AS COUNT,SUM(i.deposit) AS deposit,i.disable AS status
    FROM (SELECT DISTINCT inter.uid, b.deposit AS deposit, inter.disable
...
sub get_free_static_ip {
  ...
  $self->query("SELECT
    ...
    HAVING SeqValue NOT IN (SELECT ip from internet_main) AND IN (SELECT ip from ippools_ips)
    AND SeqValue < ?
    ORDER BY 1 LIMIT ?;",
    undef, 
