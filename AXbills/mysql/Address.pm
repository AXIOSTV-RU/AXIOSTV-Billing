...
#        b.entrances AS address_entrances
#      FROM builds b
...
$self->query("SELECT d.id AS district_id,
        ...
        b.flors AS address_flors,
        b.entrances AS address_entrances,
        u.entrance AS address_entrance,
        u.floor AS address_floor
      FROM builds b
      LEFT JOIN streets s  ON (s.id=b.street_id)
      LEFT JOIN districts d  ON (d.id=s.district_id)
      LEFT JOIN districts dp ON FIND_IN_SET(dp.id, REPLACE(d.path, '/', ',')) > 0
      LEFT JOIN users_pi u ON (u.location_id = b.id)
      WHERE b.id= ? ",
      undef,
      { INFO => 1,
        Bind => [ $id ]
...
sub street_list {
  ...
  if ($attr->{USERS_COUNT} && !$admin->{MAX_ROWS}) {
    ...
    $EXT_FIELDS_TOTAL = ', COUNT(DISTINCT b.id), COUNT(pi.uid), CONCAT(ROUND((COUNT(DISTINCT pi.uid) / 
    (SELECT SUM(b2.flats) 
     FROM builds b2
     JOIN streets s2 ON s2.id = b2.street_id
     JOIN districts d2 ON d2.id = s2.district_id
     WHERE d2.id = d.id) * 100), 2), " %")';
  }
