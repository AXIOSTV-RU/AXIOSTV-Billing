...
sub reports_channels_use{
  ...
  my $sql = "SELECT c.num,  c.name, COUNT(uc.uid) AS users, SUM(IF(b.deposit > 0, 0, 1)) AS debetors
FROM iptv_channels c
...
sub reports_channels_use2{
  ...
  my $sql = "SELECT c.num,  c.name, u.uid, ipm.disable, u.id AS user, b.deposit as deposit
              FROM  iptv_channels c
              ...
              WHERE ipm.disable=0
            UNION
            SELECT c.num,  c.name, us.uid, im.disable, us.id AS user, b.deposit as deposit
              FROM  iptv_channels c
...
sub online{
  ...
  my $WHERE = $self->search_former(
    ...
    [
      ...
      [ 'CREDIT',           'INT', 'u.credit AS credit', 1 ],
